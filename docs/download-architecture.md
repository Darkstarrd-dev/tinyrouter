...
- **Unix**（kill_unix.go:12-18）：`Getpgid` 后向进程组 `-pgid` 发 `SIGTERM`；**没有 SIGKILL 兜底**（与归档计划 §3.10 “先 SIGTERM 再 SIGKILL” 漂移，见第 11 节）。
...
| Unix 进程树杀 | “先 SIGTERM 等待 grace 再 SIGKILL”（§3.10） | 仅 SIGTERM 给进程组，**无 SIGKILL 兜底** | kill_unix.go:12-18 |
...
11. **Unix 仅 SIGTERM 无 SIGKILL**：进程组 SIGTERM 后若子进程忽略信号，可能残留 ffmpeg/yt-dlp（kill_unix.go:12-18）。
...
- `kill_unix.go`：killProcessTree / SIGTERM（12-18）、setupProcessGroup / Setpgid（22-27）。
...
| 修改进程树杀 | kill_windows.go（taskkill /T /F）+ kill_unix.go（SIGTERM 无 SIGKILL）+ executor.go cmd.Cancel（69-74、176-181） |
