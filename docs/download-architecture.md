...
>
> **最后核对：** 2026-07-19，仓库工作区（`main`）。本轮：下载模块一轮 UI 与可见性改动——(1) View Log 增加完整 yt-dlp 命令行（`executor.go:89-90` 在 `stdoutTail` 头部插入 `[command] <...>`，`GET /api/downloads/{id}/log` 响应首行即完整命令）；(2) 设置弹窗增加 File System Access Picker（`download.js:236-324` `openDownloadSettingsModal` + `fasBrowsePicker`，CSS `.dl-settings-row`/`.dl-browse-btn`，i18n `browse`）；(3) 移除主页面外部下载目录输入框，`resolveDownloadDir` 不再读 DOM（download.js:425-428）；(4) 顶部两行合并为单行 sticky `.download-toolbar`（download.js:28-86、style.css `.download-input-card` sticky）；(5) 任务卡片重构为左右分栏 `taskListItemHtml` + `taskDetailHtml` + `selectTask` + `renderTaskDetail`（download.js:539-671，CSS `.dl-task-split`/`.dl-task-list`/`.dl-task-detail` 等）。本文描述的是当时源码的实际行为，不把规划或历史设计稿当作现状。
...
5. **架构形态**：`Manager` 用单把 `RWMutex` + 缓冲 100 的 `pendingCh` + `maxConcurrent` 个 worker goroutine 实现受控并发（manager.go:24、62、97-100）；`Executor` 把 yt-dlp 当作一次性子进程执行并返回 `(filePath, log, err)`（executor.go:36-142）；失败重试由**前端重新 POST** 同一个 URL 实现（download.js:681-705），服务端没有 `RetryTask`。
...
> 服务端**没有** `RetryTask`。重试由前端 `retryDownload` 用原参数重新 `POST /downloads` 完成（download.js:681-705）。
...
| 函数 | 行 | 作用 |
|---|---|---|
| `renderDownload` | 28-86 | 渲染页面容器；顶部合并为单行 sticky `.download-toolbar`（URL + Parse + Download + Type + Quality + Container + Settings + Clear 全在一行），`.download-input-card` sticky；挂载后 `loadDownloadTasks`+`connectDownloadSSE`+`loadDownloadSettings` |
...
| `cancelDownload` / `retryDownload` / `removeDownload` / `clearCompletedDownloads` | 672-741 | 对应 API 调用；`retryDownload` **重新 POST** 原参数（服务端无 RetryTask）；`removeDownload` 删除选中项后自动选下一个 |
...
| 重试 | 计划未设计重试 | 前端 `retryDownload` 重新 POST（无服务端 RetryTask） | download.js:681-705 |
...
4. **顶部两行合并为单行 sticky**（前端）
   - `web/static/download.js`：`renderDownload`（28-86）将 `download-input-row` + `download-options-row` 合并为单个 `.download-toolbar` flex 行（URL + Parse + Download + Type + Quality + Container + Settings + Clear 同处一行）。
   - `web/static/style.css`：`.download-input-card` 设为 `position: sticky; top: 0; z-index: 10` 并加底部阴影分隔；`#dl-info-preview` 改为普通流式内容。
...
> 以上 5 项改动均已在前端代码落地（行号为本轮 `rg -n "function "` download.js 实测），服务端 `Executor`/`Manager`/API 行为未变，仅 View Log 在日志首行额外输出命令行。
