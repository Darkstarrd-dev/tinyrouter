...
> **最后核对（2026-08-04）：** Settings 新增 FileTransfer 入口与受认证保护的 `/api/filetransfer/upload` 管理端点；端点接收浏览器 multipart 文件及本机剪贴板路径，使用标准库 ZIP Deflate 打包，按 tfLink → tmpfiles.org → temp.sh → Filebin 顺序尝试，成功即返回单个下载 URL，失败汇总各服务错误。该功能不新增配置字段，临时服务列表为源码固定顺序。
