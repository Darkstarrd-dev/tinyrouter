...
> **最后核对（2026-08-04）：** Provider 新增可选 `UseCustomHeaders`/`CustomHeaders` 配置（YAML/JSON `useCustomHeaders`/`customHeaders`），经 `Registry.UpdateProvider` 持久化；旧配置缺少字段时保持 false/nil 零值兼容。额外请求头的统一应用与顺序见 `docs/proxy-architecture.md` §7.1。
