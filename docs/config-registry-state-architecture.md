...
> **最后核对（2026-08-08，Utility/FileTransfer 边界）：** FileTransfer 当前作为 Utility 的 `fileTransfer` 子工具提供 UI；Utility active header label 显示当前工具，fresh init 只显示 landing；Utility 另有独立 `editor`、`logReader`、`review` 工具，`review.js` 及 Text Review wizard 属 UI wrapper，不新增 Config/Registry/State 所有权。其后端路由组仍为 `/api/filetransfer`，在通用 `/api` 组外注册 `POST /upload`（完整路径 `POST /api/filetransfer/upload`）与 `POST /path-info`（`POST /api/filetransfer/path-info`），保留 feature gate、认证与 600MiB body cap。该上传能力不新增 `Config` 字段，也不写入 `state.yaml`；外部匿名临时服务成功与否不由本地路由保证。Download/FileTransfer 的前端生命周期 hooks 属 UI 导航事实，不改变 config/registry/state 所有权边界。相关资产路径与 Utility 导航见 `PROJECT_MAP.md` §18.2。
...
> **最后核对（2026-08-05，Settings 侧边栏弹窗控件与 Theme Stepper 数字输入框整改）：** (1) `web/static/settings_modal.js` 新增 `changeStepper`、`renderStepperHtml` 与 `renderCustomSelectHtml` 控件生成器，在 `openPortModal`、`openProxyModal`、`openRotationModal`、`openServerTimeoutModal`、`openPasswordModal` 全量引入与 Screenshot 3 表现完全一致的 `input` 统一样式文本框、Download 页面风格带展开动画与 hover 滑块的 `custom-select-wrapper` 主题下拉菜单与 Stepper 36px 数字加减输入框 `[-] [ N ] [+]`。(2) `web/static/settings_trace.js` 的 `openTraceModal` 接入 Stepper 数字输入框并采用 `settings-form-grid` 双列布局。(3) `web/static/style.css` 扩展 `.input` 与全部 `.modal`/`.dl-settings-modal` 文本框选择器，`.number-stepper` 数字居中展示（`text-align: center`），彻底纳入主题系统 (var(--input-bg), var(--glass-border), var(--text), var(--accent) 等) 的控制与 18 种变体/风格。
> **最后核对（2026-08-04，主题持久化修复）：** `config.Theme` 的 `DarkVariant`、`LightVariant`、`Style` 三个字段均由 `internal/api/settings/register.go::applyThemeUpdates` 合并并经 `SaveConfigAndReload` 持久化；此前 PATCH 漏写 `Style`，导致 `soft`/`sharp`/`compact` 仅在当前页面生效、随后被 Settings GET 的 `default` 覆盖。前端 `web/static/theme.js` 的三维属性与 `web/static/style.css` 的语义 alias token 层保持一致。
...
> **最后核对（2026-08-08，FileTransfer 上传路由）：** `/api/filetransfer` 路由组在保持 feature.FileTransfer gate、AuthMiddleware 与 600MiB `MaxBytesReader` 的前提下注册 `POST /upload`（multipart 文件/本机路径打包 ZIP 后按顺序尝试匿名临时服务；外部服务成功与否不由本地路由保证）及 `POST /path-info`（Windows 剪贴板路径递归容量查询）；该组位于通用 `/api` 组外以绕过 1MiB 限制。该改动不新增配置字段。
> **最后核对（2026-08-06，归档配置与运行时回调）：** `Config.Archive`（`ArchiveConfig{SevenZipPath,RarPath,TempDir}`，`yaml/json:"archive,omitempty"`）与 `config/paths.go::ResolveArchiveTempDir` 新增（空→`{configDir}/archives`）。Settings GET 返回 `archive:{sevenZipPath,rarPath,tempDir}`；PATCH 接受 presence-aware `archivePatch`（全指针字段：未发送不覆盖、空串=显式清除回 env/PATH），经 `internal/api/settings/register.go::applyArchiveUpdates` 逐字段合并后调 `apibase.Deps.ArchiveSettingsFn(cfg.Archive)` 推给 `archivetool.Runner`（换配置 + 失效工具 probe 缓存）。**归档字段绝不整结构覆盖**（沿用 rotation/theme 的 presence-aware 纪律）。注意：归档能力**不落盘运行时状态**（TempStore 是磁盘 workspace，注册表在内存，重启由 Scavenge 清孤儿），`state.yaml` 无归档段。
...
| `ArchiveConfig` | types.go（2026-08-06 新增，`Config.Archive`，`yaml/json:"archive,omitempty"`） | `SevenZipPath` / `RarPath` / `TempDir`——全可选：空工具路径运行时回退 `SEVENZIP_PATH`/`RAR_PATH` env → PATH；缺失工具不阻塞启动，只禁用对应归档能力。`TempDir` 空→`{configDir}/archives`（`config/paths.go::ResolveArchiveTempDir`），相对拼 configDir，绝对原样 |
...
| `Config` | types.go:266-281 | 顶层：上述所有子结构 + `Port` / `ConsoleLogMaxLines` / `UsageRingSize` / `EnablePlayground` / `QuickSlotOnly`（`bool`，`yaml/json:"quickSlotOnly"`，控制 `/v1/models` 仅返回 QuickSlot 模型）/ `Providers` / `Combos` / `QuickSlots` / `Shortcuts`（`ShortcutsConfig`，`yaml/json:"shortcuts,omitempty"`，仅存用户覆盖；未出现该段时回退前端系统预设）/ `Archive`（`ArchiveConfig`，`yaml/json:"archive,omitempty"`，2026-08-06 新增，见上表行） |
...
- `defaults.go`：DefaultServerConfig(11-18)、FinalizeServerConfig(22-36)、DefaultConfig(39-64)、finalizeConfig(69-146：enablePlayground 探测 82-84、state_persist 探测 87-89、quota 默认 96-102、download 段探测 114-117、key 解密 130-144)。Provider 自定义请求头无默认回填，旧配置缺省为零值。
...
- `customheaders.go`：`Config`/`Apply`（开关控制、名称/CRLF 校验、Set 覆盖语义）；Provider 自定义头由 config 字段持久化，经 registry/API/proxy 调用方应用。
- `customheaders_test.go`：禁用/空配置、覆盖、CR/LF 与非法名称测试。

**internal/registry：**

- `registry.go`：Registry(11-16)、New(19-26)、reloadStatesLocked(28-54)、stateKey(56-58)、Config(61-65)、Reload(68-73)。
- `providers.go`：ListProviders/HasProvider/GetProvider/GetProviderByPrefix(11-54)、AddProvider(56-70)、UpdateProvider(72-94，含 UseCustomHeaders/CustomHeaders 写回)、DeleteProvider(96-113)、UpdateProviderStrategy(116-127)。
...
| 新增/修改归档配置（ArchiveConfig） | `config/types.go`（`ArchiveConfig` + `Config.Archive` tag）+ `config/paths.go`（`ResolveArchiveTempDir`）+ `internal/api/settings/register.go`（GET `archive` 对象 + presence-aware `archivePatch`/`applyArchiveUpdates` + `ArchiveSettingsFn` 推送）+ `internal/api/apibase/deps.go`（`ArchiveSettingsFn` 字段）+ `internal/app/app.go`（`archivetool.NewRunner` 装配）。**不新增 `state.yaml` 持久化**；归档字段 PATCH 必须保持 presence-aware（未发送不覆盖、空串=清除）。详见 `docs/archive-architecture.md` §6 |
| FileTransfer 上传与路径容量 | `internal/filetransfer/upload.go`（`Upload`/`PathInfo`/`localPathSize`）+ `internal/api/router.go`（`/api/filetransfer` 路由组：`POST /upload` 与 `POST /path-info`，feature gate、认证与 600MiB body 上限）+ `web/static/filetransfer.js`（multipart 上传、剪贴板容量、接收区域拖拽保护、Clear、XHR 进度）+ `web/static/i18n.js`/`style.css`；上传成功依赖外部临时服务，路由本身不保证外部服务可用 |
