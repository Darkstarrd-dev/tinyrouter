...
> **最后核对（2026-08-05，FileTransfer 交互修复）：** `/api/filetransfer/path-info` 为 Windows 剪贴板路径提供递归文件容量查询，Settings FileTransfer modal 显示准确总容量；前端拖拽保护仅允许投放到接收区域，新增 Clear 重置和 XMLHttpRequest 上传进度条。该改动不新增配置字段。
...
| FileTransfer 交互与路径容量 | `internal/filetransfer/upload.go`（`PathInfo`/`localPathSize`）+ `internal/api/router.go`（`/api/filetransfer/path-info`）+ `web/static/filetransfer.js`（剪贴板容量、接收区域拖拽保护、Clear、XHR 进度）+ `web/static/i18n.js`/`style.css` |
