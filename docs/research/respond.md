...
输入拼接（user message）：`scenario.coreSeed + world`（来自档案；无档案则取各窗口 systemPrompt 首 200 字并集）＋ `director.plotOutline`（M1 才有，"有则遵循、无则自由裁量"由提示词第 3 条承接）＋ **timeline 尾部最近 20 条**（复用 `pgAutoChatMaybeSummarize` :760 已有的 timeline→文本序列化逻辑；**pass 条目保留并渲染为『<角色名> 选择不发言』**——这是停滞判断的关键输入；error 条目跳过）＋ 旁白计数与上次旁白。

### 5.3 触发机制（D4 决策展开）：事件计数驱动

在 `pgAutoChatOnFinish` 的**两个分支**各加一行守卫式调用——正常分支紧邻 :372 `pgAutoChatMaybeSummarize` 之后；pass 分支在 :337 `pgAutoChatCheckAllDone()` 之后、:338 `return` 之前（pass 是更强的停滞信号，必须推进评估节奏）：
...
```
repliesSinceLastEval++   // normal 回复与 <pass/> 均计数——pass 是更强的停滞信号，必须推进评估节奏
...
fetch 判决（stream:false；AbortController 挂 pgDirectorAbort；**超时 30s**——setTimeout 触发 abort，超时即清 evalInFlight 并静默跳过本周期）
  .then(decision):
...
fetch 生成（stream:false；narratorModel；**超时 60s**——超时触发 abort、清 narratorPending 并静默跳过，避免挂起阻塞 CheckAllDone）
...
1. **timeline 追加函数复用（无需导出）**：`pgAutoChatAppendTimeline(sender, senderType, winIdx, content, status)`（pg-autochat.js:89）是 vanilla JS 顶层函数声明，**本即全局可调用**，director 直接复用即可，保证 `timelineId` 单调性由唯一入口维护。
...
| **终止判断** `pgAutoChatCheckAllDone`（:379-402） | 循环体前加两个短路：`if (pgDirectorEvalInFlight() || pgDirectorNarratorPending()) return;`（守卫式，函数不存在则跳过） | 防止竞态：所有窗口恰好 idle、但判决/旁白在途——若此刻 finish，旁白会注入到已结束的会话。director 活动期间视为"系统仍忙"。fetch 超时（§5.3/§5.4）保证 pending 标志不会因挂起而永久阻塞终止 |
...
| `pg-autochat.js` | ① `pgAutoChatRenderPerspective` 加 narrator 分支（:125 旁）② `pgAutoChatOnFinish` 正常分支(:372后)与 pass 分支(:337后)各加 `pgDirectorOnAgentReply` 守卫调用 ③ `pgAutoChatCheckAllDone` 头部加 pending 短路、allDone 后加 `pgDirectorOnBeforeFinish` 守卫（:379/:399）④ start/stop/finish 各加 `pgDirectorReset` 守卫 ⑤ `pgAutoChatMaybeSummarize` 序列化加 narrator 行（注：`pgAutoChatAppendTimeline` 本即全局，无需导出） | ~13 行 |
...
8. **director/narrator fetch 挂起**：若判决或旁白的 fetch 因网络/模型卡死而挂起（非主动 abort），`evalInFlight`/`narratorPending` 将保持 true，§5.5 的终止短路会使对话无法自然结束。缓解：两次 fetch 均加超时（判决 30s、旁白 60s，见 §5.3/§5.4），超时即 abort、清标志、静默跳过本周期——旁白丢一拍无碍剧情，下个计数周期自然重来。
