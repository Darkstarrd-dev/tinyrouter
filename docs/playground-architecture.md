...
> **最后核对（2026-08-08，Utility 编辑器本地工作区与 RootStatic 资产）：** 当前管理 UI 顶层 Download/GIF 由 Utility 菜单承载；Utility 子工具为 `editor`、`logReader`、`review`、`gif`、`download`、`fileTransfer`，fresh init 只显示 landing，不预选工具。Editor、Log Reader、Text Review 资产位于 `web/static/utility/editor/`，由 `RootStatic` 提供给默认与 Playground 构建；`web/playground/static-pg/editor` 仅保留历史迁移背景。Editor 依次加载 `editor-state.js`、`editor_workspace.js`、`editor_commands.js`、`editor_markdown.js`、`editor_layout.js`、`editor.js`、`editor_shell.js`、`editor-logs.js`，随后加载 Text Review 模块。当前 Editor 是 StackEdit-inspired 的本地工作区：IndexedDB（不可用时 in-memory fallback）含 Welcome/Trash/Temp，支持文件 CRUD/move/restore/current/expanded metadata；shell 提供 Explorer、导航/格式控件、单一主 textarea + preview、TOC/status/control toggles，以及本地 import/export/print。Editor/Log Reader/Text Review 是独立 Utility tools；不提供 cloud/sync/accounts/comments/PDF/Pandoc，也不声称 KaTeX/HTML export。`suspendEditor`/`resumeEditor` 是 Editor Utility 生命周期边界；`POST /api/editor/open` 与 `POST /api/editor/save` 保持不变。
...
> **最后核对（2026-08-08，Monitor Recent Requests 详情弹窗共享边界）：** 管理端 Recent Requests 详情弹窗固定六个 section：`Request Info`、`Request`、`Request Headers`、`Response Headers`、`Status`、`Response Body`；`Status` 为无折叠、无 Pretty/Raw/Copy 的静态状态行，其余五个 section 默认折叠并提供 section 级 Pretty/Raw/Copy，字段继续提供字段级 Pretty/Raw/Copy。section Raw 直接显示该 section 的原始文本，字段 Raw 直接显示字段原文，只有 Pretty 执行 JSON/Markdown/SSE 解析。section header 与 field header 两级 sticky。Playground 详情继续复用 `info_common.js` 的共享能力边界，不改变其既有调用方默认语义。
>
>
...
> **最后核对（2026-08-07 Playground CSS 首轮高置信度 Token 清理）：** 首轮 CSS 清理仅修改 `web/playground/static-pg/playground.css` 共 **13 行**：将命中处硬编码色值替换为已有语义 Token（`--text-on-accent`、`--code-surface`、`--code-text`、`--danger`），仅限高置信度匹配，未改变任何模块/DOM/JS/加载顺序；本轮**未**进行 CSS 拆分、`!important` 清理，也**未**执行浏览器/HTTP 验证（留待后续轮次）。
> **最后核对（2026-08-07 Playground CSS 第二批高置信度 Token 清理）：** 第二批 CSS 清理在 `web/playground/static-pg/playground.css` 完成 **3 处** `--text-contrast` 替换：`.pg-send`、`.pg-send.stop`、`.pg-image-rem:hover` 三处按钮表面（accent 渐变与 danger 悬停态）的硬编码 `color:#fff` 改为 `var(--text-contrast)`，均属高置信度匹配；本轮同时推进了核心 `web/static/style.css` 的 Token 清理，其详细事实以 `css_upgrade_plan.md` 为准。本轮**未**改变任何 DOM/JS/加载顺序，也**未**进行 CSS 拆分、`!important` 清理或浏览器/HTTP 验证（留待后续轮次）。
> **最后核对（2026-08-07 Playground CSS 第三批 Token 清理）：** 第三批 CSS 清理涉及 Playground 的 `.pg-code-warning` 与 `.pg-code-expand-btn` 两处 `border-radius:6px` → `var(--radius-sm)`；同时核心 `web/static/style.css` 也进行了第三批 Token 清理，详细范围见 `css_upgrade_plan.md` §1.8。本轮不改 DOM/JS/加载顺序，不拆 CSS，不增删 `!important`；构建级门禁已通过，HTTP/浏览器验证仍未执行。
> **最后核对（2026-08-07 Playground CSS 第四批 Token 清理与 Phase 2 第一批）：** 第四批完成 Playground 5 处 `color:#fff` → `var(--text-contrast)`（`.pg-input-thumb-del`、`.pg-gc-new-msgs`、`.pg-model-picker-item.selected`、`.pg-search-history-del`、`.log-mode-btn.active`，均为 accent/danger 表面上文字）；Phase 2 第一批按「同 selector 后规则完全覆盖」证明删除 Playground 3 条规则（`.pg-winbar`、`.pg-param-row label`、`.pg-param-row input:focus`）并收窄 `.pg-param-row` 3 条重复声明（保留 `font-size`/`color`）；不改 DOM/JS/加载顺序、不拆 CSS、不增删 `!important`；构建级门禁已通过（feature test / 双构建 / media-bridge / 57 JS check / CSS 平衡），HTTP/浏览器验证仍未执行。详细记录见 css_upgrade_plan.md §1.11/§1.12。
> **最后核对（2026-08-07 Playground CSS 第五批：DESIGN 一致性核验 + 圆角 Token 化）：** 第五批完成 `DESIGN.md` 与全部 token 层（root/light/9+9 variant/4 style/3 字号）一致性核验（结论完全一致）；Playground 侧圆角 Token 化 3 处（`.pg-bubble kbd`/`details`/`code` 的 4px/6px → `--radius-xs`/`--radius-sm`，低风险选点，避开 GIF/Text Review/全屏区域）；不改 DOM/JS/加载顺序、不拆 CSS、不增删 `!important`；构建级门禁已通过（feature test / 双构建 / media-bridge / 57 JS check / CSS 平衡），HTTP/浏览器验证仍未执行。详细记录见 css_upgrade_plan.md §1.14。
> **最后核对（2026-08-07 Playground 真实 HTTP/浏览器 smoke 首次执行）：** 以 `-tags playground` 构建在 localhost:21337 验证：Playground shell 加载 4 个 CSS（/style.css、/playground.css、/vendor/pg-highlight-theme.css、/vendor/katex.min.css）全 200、默认 shell 仅 /style.css 且 pg 元素缺席；0 console/page error（唯一 request failure 为 `/api/monitor/events` SSE 导航中止，属应用行为）；主题矩阵 9 组合（dark/light × default/tokyo-night/cool × sharp/soft/compact × font s/m/l）computed 值与 token 层一致；desktop 1440 / tablet 768 / mobile 390 无横向溢出；modal 无裁切、Download 页 dropdown 在视口内且 bg=--surface-overlay；reduce 媒体匹配下动效有意不变。证据存 tmp/css-verify/evidence/（gitignored）。详细记录见 css_upgrade_plan.md §1.19。
> **文档定位：** Playground 前后端实现的 canonical 架构事实基线。
> **最后核对（2026-08-06 GIF 编辑器 步骤面板 DOM ID 绑定修复与功能解锁）：** 修复了导入完成后步骤 2 (Frame Editing) 和步骤 3 (Output Settings) 面板无法解锁的 Bug：(1) 修正 DOM ID 绑定规则 `dom.panelStep2 = core.byId('gif-panel-step2')` 和 `dom.panelStep3 = core.byId('gif-panel-step3')`（原先缺少 `gif-` 前缀且缺少 `panelStep3` 的抓取导致拿到 `null`）；(2) 在 `updateSelectionUI()` 中同步切换 `dom.panelStep2` 与 `dom.panelStep3` 的 `gif-edit-blocked` CSS 类（消除 `pointer-events: none` 禁用状态）；(3) 在素材提交 `commitDraft` 时显式触发 `updateSelectionUI(0)`，确保素材导入成功后全套编辑与导出功能即刻完全激活可交互。
...
> **最后核对（2026-08-06 GIF 编辑器页面 UI 调整与预览缩放重构）：** 按照用户需求完成 3 组界面调整：(1) 左侧边栏移除 "GIF Frame Editor Pro Max" 标题，将 "重置" (Reset) 按钮右对齐并入 "1. 导入素材" 标题行；(2) 导入 Modal 弹窗控件与文字样式参照项目 Settings 通用弹窗，隐藏原生 number 输入框 spin 按钮，Import Scale 改为滑条控制 (范围限制 10%–100%，消除上采样放大)，第一行 Source Resolution 在 Scale!=100% 时自动追加 " -> 缩放后分辨率"；Import FPS, Start (ms), End (ms) 三字段改为 3 列垂直 label+input 并排布局；Actual Frames 与 Duration 放置于同一行展示；(3) 彻底修复布局盒模型（`#page-content` 与 `.gif-workspace` 显式强制 `height: 100%`，右侧采用 `grid-template-rows: 1fr auto`），使得左侧 sidebar 内容多时自动出现垂直滚动条，右侧下方 155px 固定的 thumbnail 时间线和控制栏永远贴在右底部不会被挤出视口；Modal 关闭后通过延时 `resetView` 自适应画面；重新定义底部 Toolbar 的 Zoom 滑条作用，解绑 timeline 缩略图宽度，改为控制主舞台预览画布 `core.state.scale` 的 10%–300% 缩放与文本联动；在 Extraction / Loading 渲染导出全过程中增加捕获阶段键盘监听，锁死按键响应防误触退出。
> **最后核对（2026-08-06 GIF 编辑器受控模块重构与 Import Modal）：** `web/static/` 下 GIF Editor 页面按 `gif_upgrade.md` 实施重构拆分为 `gif-editor-state.js` / `gif-editor-import.js` / `gif-editor-timeline.js` / `gif-editor-playback.js` / `gif-editor-export.js` 和 `gif-editor.js` 六个文件（在 HTML 入口依次加载）。图片/GIF/视频导入统一进入 Import Modal（支持起点/终点双手柄滑块、数值输入、比例、FPS、实际帧数与时长联动统计，取消不影响已有工程）；时间线支持 50%–200% 动态缩放和独立 scroll 视图；底栏 Toolbar 新增 First/Prev/Play/Pause/Next/Last 焦点与按 delay 自动播放控制；MediaBridge 结果交接保留。
...
> **最后核对（2026-08-04）：** Image 模式新增 `comfyui` 协议：端口输入连接本机 ComfyUI；`internal/api/comfyui/register.go` 提供 `POST /api/comfyui/proxy`（仅 `127.0.0.1:{port}`、GET/POST、响应透传与重定向端口约束）及 `GET /api/comfyui/active`（Windows Comfy Desktop 活动 Tab 桥接）；`pg-comfyui.js` 通过 `/system_stats`、`/models/*`、`/object_info/KSampler`、`/userdata`、`/history` 动态发现模型/工作流，解析 API 工作流生成参数控件，提交 `/prompt` 后轮询 `/history`，经 `/view` 转 base64 data URL 并复用 `/api/save-image`。浏览器不再直连 ComfyUI WebSocket，避免 Origin 校验差异。
...
> **最后核对（2026-08-05 Header 组件高度统一 86px 与动态折叠）：** Header 页面选择器（`.top-header-nav` 及 `.nav-item`）样式重构为完全跟随动态主题系统（`data-theme` Mode + Variant）。已移除 `.top-header-nav` 外框背景、边框、外阴影与外边距 (`border: none; background: transparent; box-shadow: none; padding: 0`)，仅保留独立的按钮元素；亮色模式隐藏按钮间菱形块 (`display: none`)。Header 所有组件（Page 选择器、QuickSlot 容器、Logo 容器、关闭按钮）统一对齐至 **`86px`** 高度，Header 容器固定 **`98px`** 高度 (`padding: 6px 16px; height: 98px`)，彻底解决组件间高度不一致与隐藏时的高度跳变；通过 `auth.js` 中的 `setupHeaderResponsive()` 实时精确计算可用宽度动态隐藏：(1) 先隐藏关闭按钮，(2) 再隐藏 Logo 容器，(3) 最后隐藏 QuickSlot；Page 选择器原样保留。
> **最后核对（2026-08-05 Playground 模式选择器样式与主题联动）：** `.pg-winbar-header` 高度精确修正为 **`37px`**（`.pg-mode-toggle` `height: 100%`），使底部分割线 `border-bottom` 的 Y 轴坐标与左侧 `div.pg-pane-head` 完美的绝对像素连贯水平对齐。`--pg-mode-*` 变量绑定动态主题系统（`data-theme`），并为 4 个模式分配专属高鲜度色彩（Normal `#38bdf8`、Search `#a78bfa`、Image `#f472b6`、AutoChat `#fbbf24`）。暗色模式保留霓虹发光与中高对比度未选中字色，亮色模式去除 `text-shadow` 模糊扩散与内阴影色彩发光。
...
> **历史与当前边界（2026-08-08）：** 上述 2026-07-26 Editor/Clean-mode 记录仅保留迁移背景；Clean tab、Gallery↔Editor toggle、旧 F6 入口以及旧两 pane raw/parsed Editor shell 均不是当前契约。当前 Text Review 由独立 Utility `review` tool + `web/static/utility/editor/review.js` wrapper 提供。当前 Editor 是 RootStatic 提供的 StackEdit-inspired 本地工作区，具体模块与生命周期见 §17。
...
- **Image 模式**（`mode = 'image'`）：现有 GPT/xAI/ModelScope 图片协议继续通过 `/v1/images/*`；新增 `comfyui` 协议后，协议筛选器显示端口连接面板，不使用 TinyRouter `/api/models` 的模型选择器。用户输入端口并连接后，前端通过 `POST /api/comfyui/proxy` 读取 ComfyUI `/system_stats`、`/models/*`、`/object_info/KSampler`、`/userdata` 和 `/history`，并请求同源 `GET /api/comfyui/active` 获取 Comfy Desktop 当前 Tab 路径；通过 `/userdata/workflows/...` 读取工作流，活动 Tab 优先，其余已保存与历史候选按来源、workflow id/规范化签名去重。无任何候选时可粘贴 API-format JSON。`pg-comfyui.js` 识别 `UNETLoader`/`CLIPLoader`/`VAELoader`/`KSampler`/`EmptyLatentImage`/`CLIPTextEncode`/`SaveImage` 输入生成下拉与输入框，生成时把输入栏文本写入正向 `CLIPTextEncode`，POST `/prompt` 后同源轮询 `/history/{prompt_id}`，再经 `/view` 转 base64 data URL，复用消息图片渲染和 `/api/save-image`。代理固定 loopback 目标…
