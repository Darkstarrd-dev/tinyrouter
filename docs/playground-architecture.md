...
> **最后核对（2026-08-07 GIF 编辑器侧栏步骤 2/3 控件接线修复）：** `web/static/gif-editor/gif-editor.js` 的 `pageTemplate()` 以新 ID（`gif-global-crop-btn`/`gif-text-input`/`gif-text-color`/`gif-text-stroke-color`/`gif-layer-scale`/`gif-delete-layer-btn`/`gif-scale-percent`/`gif-apply-scale-btn`/`gif-output-width`/`gif-output-height`/`gif-output-scale-slider`/`gif-output-scale-val`/`gif-sample-interval`/`gif-quality-val`/`gif-export-gif-btn`/`gif-export-frames-btn`/`gif-export-sprite-btn`/`gif-crop-start`/`gif-crop-end`/`gif-delete-range-btn`/`gif-keep-range-btn` 等）为唯一事实来源，入口 `cacheDom()`/`bindEvents()`/`getTargetIndices()` 与 `gif-editor-export.js` 全部对齐（原 ID 全部失效 → 步骤 2/3 控件静默无响应）；模板补齐全局裁剪微调面板（`gif-crop-panel`/`gif-crop-slide-x/y/w/h`/`gif-crop-num-x/y/w/h`/apply/cancel，复用 `.gif-crop-panel.active`）与精灵图导出按钮（`gif-export-sprite-btn` 接线既有 `exportSprite()`）；移除无处理死单选（`gif-export-fmt`）与 `syncLayerBtn`/`syncLayer()`（模板以删除图层按钮取代）；新接线描边色、图层缩放（`layerBase` 基线，选中/命中重置 1）、删除图层、百分比应用到输出尺寸（修正输出缩放滑条 100 视为 100× 的 bug）；范围作用域改 `gif-crop-start`/`gif-crop-end` 双输入并随单选切换 `gif-crop-range-inputs`。i18n 补 `gifEditorCancelCrop`/`gifEditorCropX/Y/W/H`/`gifEditorStroke`/`gifEditorDeleteLayer`/`gifEditorApplyAll`/`gifEditorExportSprite`/`gifEditorColor`（en/cn）。浏览器冒烟（headless Chrome + playground 构建）全过：三导出 + 0 console error，详见 `gif_implented.md` 顶部。
>
