...
> - **确认关机/全局弹窗键盘焦点与交互增强：** `confirmModal`（含 Esc 触发的 Shut Down 确认弹窗）打开后自动聚焦 Confirm（确定）按钮，添加高亮 focus 边框发光；支持按 `ArrowLeft` / `ArrowRight` / `ArrowUp` / `ArrowDown` 方向键在 Confirm（确定）与 Cancel（取消）之间切换焦点；按 `Enter` 键触发当前聚焦按钮的确定/取消操作。
