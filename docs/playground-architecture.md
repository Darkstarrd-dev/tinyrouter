...
> **2026-08-04 Playground 主题 Token 整改：** `playground.css` 的消息气泡、错误/系统状态、代码块、Reasoning、Debug、SSE 区域改用 `style.css` 语义 surface/status/code/border Token；`style.css` 仍先加载，`playground.css` 后加载并保留 Playground 全屏布局契约。
