...
>
> **最后核对（2026-08-05 Comfy Desktop Active Tab）：** `pg-comfyui.js` 连接时优先请求同源 `GET /api/comfyui/active`；Windows 后端扫描 Comfy Desktop `Local Storage/leveldb` 的 `Comfy.Workflow.LastActivePath`，返回当前 Tab 对应的工作流路径，再通过 ComfyUI `/userdata/workflows/...` 读取并经同一 UI graph→API prompt 转换显示为 `Current Tab`。活动 Tab 桥接不可读时回退到 `/userdata?dir=workflows&recurse=true&split=false&full_info=true` 的已保存工作流，再合并成功历史候选；候选按来源、workflow id/规范化签名去重。REST API 本身不暴露浏览器内存 `activeWorkflow`，本地桥接仅覆盖 Comfy Desktop Windows 存储格式；未保存 Draft 仍需后续单独桥接。空态文案为“未找到已保存或已执行的 Tab”。
