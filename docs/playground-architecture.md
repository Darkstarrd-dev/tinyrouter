...
| 新增/修改 GIF 编辑器页面（`web/static` 全局 SPA 页） | `gif_implented.md`（实施入口，§4.3/§4.6/§7/§8 与 ADR 10-11）、`web/static/gif-editor.js`（`renderGifEditor`/`cleanupGifEditor`；常量 `MAX_FILE_BYTES`/`EXPORT_MEM_LIMIT` 与 `exportMemCheck`；虚拟化时间轴 `timelineWindow`/`thumbCache`/`THUMB_CACHE_MAX`，容器委托交互）、`web/static/i18n.js`（`gifEditor*` 键）、`web/static/style.css`（`gif-*` 样式，`.gif-timeline-track` 绝对定位几何）、`web/static/{index.html,index-nopg.html}`（第 6 按钮 + 三 script）、`web/static/app.js`（`case 'gif'` + cleanup）、PROJECT_MAP §18.2/§24 |
