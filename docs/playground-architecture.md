...
>
> **最后核对（2026-08-05）：** `pg-comfyui.js` 模板来源扩展——连接时除 `/history` 外，另经 `/userdata?dir=workflows&recurse=true&split=false&full_info=true` 列出 `user/default/workflows/` 下的已保存工作流并按 `modified` 倒序置于模板列表首位（`prompt_id` 加 `file:` 前缀防与 history UUID 冲突）。已保存文件是 UI graph 格式（`{id, nodes[], links[], definitions.subgraphs[]}`），提交 `/prompt` 前在 `pgComfyGraphToPrompt` 中转换为 API 格式：widget 值按节点 widget 定义顺序对齐（`control_after_generate` 独占槽位，`/object_info` schema 白名单过滤 `upload` 等非 API 输入），link 输入解析为 `[源节点, 源槽]`（Reroute 链回溯到最终源），subgraph 节点（UUID 类，`definitions.subgraphs` 内）递归展开——端口值注入内层目标、输出端口把消费方重连到内层生产者。转换失败的工作流跳过并 console.warn，不影响连接。空态文案 `pgComfyTemplateNone` 同步改为"未找到已保存或已执行的工作流"。
...
> **最后核对（2026-08-05 Header 页面选择器样式与主题联动）：** Header 页面选择器（`.top-header-nav` 及 `.nav-item`）样式重构为完全跟随动态主题系统（`data-theme` Mode + Variant），移除所有硬编码颜色。外框背景、边框、按钮渐变与中心菱形装饰均经由 CSS3 `color-mix(in srgb, ...)` 原生算式自动从主题底色派生。为 5 个页面按钮分配各自全局独立的炫彩发光色（Monitor `#38bdf8` 极光冰蓝、Endpoint `#f472b6` 樱花绯粉、Playground `#fbbf24` 璀璨琥珀、Gallery `#f97316` 秋阳暖橙、Download `#34d399` 翡翠薄荷）。暗色模式保留炫彩发光文字与菱形染色；亮色模式精细化去除文字模糊扩散、发光伪元素与菱形染色，保持纯粹清晰高颜值的极简设计。
...
- **Image 模式**（`mode = 'image'`）：现有 GPT/xAI/ModelScope 图片协议继续通过 `/v1/images/*`；新增 `comfyui` 协议后，协议筛选器显示端口连接面板，不使用 TinyRouter `/api/models` 的模型选择器。用户输入端口并连接后，前端通过 `POST /api/comfyui/proxy` 读取 ComfyUI `/system_stats`、`/models/*`、`/object_info/KSampler` 和 `/history`，工作流模板按"已保存工作流（`/userdata?dir=workflows` 列出 `user/default/workflows/*.json`，`modified` 倒序，`file:` 前缀 id）→ 历史成功 prompt（`/history`）"顺序合并；已保存文件是 UI graph 格式，经 `pgComfyGraphToPrompt` 转成 API 格式（widget 顺序对齐 + `/object_info` schema 白名单 + Reroute 回溯 + subgraph 递归展开），无任何模板时可粘贴 API-format JSON。`pg-comfyui.js` 识别 `UNETLoader`/`CLIPLoader`/`VAELoader`/`KSampler`/`EmptyLatentImage`/`CLIPTextEncode`/`SaveImage` 输入生成下拉与输入框，生成时把输入栏文本写入正向 `CLIPTextEncode`，POST `/prompt` 后同源轮询 `/history/{prompt_id}`，再经 `/view` 转 base64 data URL，复用消息图片渲染和 `/api/save-image`。代理固定 loopback 目标、仅允许 GET/POST，并限制重定向留在原端口；不依赖 ComfyUI CORS 或浏览器 WebSocket Origin 行为。'
