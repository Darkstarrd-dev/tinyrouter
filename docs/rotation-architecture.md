...
>
> **最后核对：** 2026-07-19，仓库工作区（`main`）。本文描述的是当时源码的实际行为，不把规划或历史设计稿当作现状。
...
7. **NIM 全池重置绕过速率限制：** 当所有 key 用尽本轮计数（无 429 冷却）时，`filterNIMCandidates` 直接 `resetNIMRequestCount` 全池（nim.go:172-176），不引入任何冷却即开启新一轮。
