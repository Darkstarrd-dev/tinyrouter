...
> **最后核对（2026-08-03）：** Quota Monitor 主行 latency/avg-speed 首次渲染与周期回填路径已按 `usage_quota.js::refreshAllKeyDetails` 核对。
...
> **2026-07-29 更新（Usage 前端拆分与重构，行为不变）：** `web/static/usage.js`（1634 行）拆为 6 文件，`index.html`/`index-nopg.html` 加载顺序 `usage_state→usage_io→usage_quota→usage_recent→usage_modal→usage.js`（`//go:embed all:static` 自动纳入）。**条目合并抽取**：`refreshQuotaData` 与 `renderUsage` 原各自的去重/inflight 合并/terminal 保留逻辑合并为 `usage_io.js::mergeUsageEntries`（O(n) Map 索引替代 `.find()`，**行为不变**——§8 Recent Requests 去重/保活/会话分组描述仍准确）。**Quota Monitor 性能**：`usage_quota.js::updateQuotaTable` 停止每周期 sub-row 重建，改条件性 DocumentFragment 整组重排（主行+其 `.quota-key-row` 兄弟一并迁移，稳态 0 次 DOM 移动，sub-row 不孤立）；`refreshAllKeyDetails` 为回填未展开主行的 latency/avg-speed 会请求所有 quota bar，cache 新鲜（TTL 内）的行跳过；展开仍由 `toggleQuotaRowExpand` 直接 `fetchModelKeyDetail`（绕过 TTL），sub-row 渲染仍由 `expandedModels` 门控。**i18n/清理**：`usage_modal.js` Info Modal 硬编码英文走 `t()`（`i18n.js` 新增 `loadFailed`+17 `info*` 键）；删除死代码 `lastUsageSig`/`lastQuotaSig` + 4 个废弃 recent-requests-modal 函数 + `showUsageEntryInfo(ts)`。**文件锚点迁移**（供下方变更维护清单的 `usage.js` 引用对照）：`mergeUsageEntries`/`refreshQuotaData`/`applyUsageSSEHandlers`/`handleRequestStart`/`handleRequestDone`/`handleRequestChunk`/`handleRequestTTFT`/`handleRequestTokens`→`usage_io.js`；`renderUsageRow`/会话分组（`renderRecentRowsGrouped`/`toggleSessionGroup` 等）/`updateUsageSummary`→`usage_recent.js`；`renderQuotaRow`/`patchQuotaRow`/`updateQuotaTable`/`toggleQuotaRowExpand`/`fetchModelKeyDetail`/`refreshAllKeyDetails`→`usage_quota.js`；`showUsageEntryInfoById`/`showUsageEntryInfoWithData`/`loadTraceDetails`/`updateStreamingModalResponse`→`usage_modal.js`；`displayModelName`/`buildModelIdToAlias`/`formatBody`/`sortEntriesByTimeDesc`→`usage_state.js`；`renderUsage`/`updateProcessingLatencyCells`/`startUsageRefresh`/`stopUsageRefresh`/`ensureProcessingTimer`/`resetQuotaTimers`→`usage.js`。
> **2026-08-03 更新（Quota Monitor latency/avg speed 修复）：** `usage_quota.js::refreshAllKeyDetails` 必须为每个 quota bar 请求 `/usage/model-keys`，因为顶层 latency/avg-speed 单元格初始为 `—`，且 `patchQuotaRowActiveMetrics` 只在 `fetchModelKeyDetail` 完成后回填。此前为优化展开行而跳过 collapsed rows，导致未展开模型永远不请求详情、两列永久空白；现由 `renderUsage` 在首次绘制后立即触发，周期刷新继续使用 `KEY_DETAIL_TTL`，并保留 `expandedModels` 门控仅用于 sub-row 渲染。
> **2026-08-03 更新（Quota Monitor 首次加载空白修复）：** `/api/usage/quotas` 现在把 `EntryTracker` 中仍在处理的非 Playground 请求按 provider/model 加入 provisional quota bar。此前首次打开 Monitor 时请求已进入 Console/Recent Requests 但尚未完成，`QuotaTracker` 与累计 `ModelStats` 都尚未产生该模型，API 返回空数组；请求完成后才出现 quota 行。
> **2026-07-31 更新（Quota 单元格显示格式 + exhausted key 过滤）：** `usage_quota.js`(a) `renderQuotaKeyRows` 跳过 `data.hasQuota && k.hasQuota && k.modelRemaining === 0` 的 key（已耗尽 key 不再显示在展开的 per-key 列表中，其配额贡献已计入 provider 级聚合）；(b) 新增 `formatQuotaCell(bar)` 辅助，渲染顶层配额单元格为 `<span class="quota-success">successCount</span> / <span class="quota-capacity">totalCapacity|∞</span>` 加可选红色 `<span class="quota-error-badge">errorCount</span>`（仅 errorCount>0）。`renderQuotaRow`/`patchQuotaRow`（现用 innerHTML）消费之。unlimited 模型显示 `successCount / ∞`。`style.css` 新增 `.quota-success`（绿 `--accent2`）、`.quota-sep`（淡化）、`.quota-capacity`（二级色）、`.quota-error-badge`（红 badge）类；列 4 宽度从 84px 扩至 110px。`getQuotas`（`internal/api/usage/register.go`）现从 per-key `ModelQuotas`（`GetQuota` 锁安全读取）重算 `TotalUsed`/`TotalCapacity`，覆盖 `QuotaTracker` 的纯会话聚合，使重启后 `QuotaTracker` 为空时 exhausted key 的配额贡献仍计入 provider 级总量。
...
> **会话连续性推理（2026-07-26 新增）：** `forward.go::sessionKeyFromMessages` 从请求体 `messages` 的 system 消息 + 首条 user 消息内容（各截 4096 rune，null 分隔）算 FNV-1a 64→8 hex 指纹，作为推断的 per-session key（空=单发/未分组）。`handleProxy`（`forward_request.go`）计算后经 `forwardWithRetry`/`handleCombo`/`retry.go` handlers/`stream.go` 一路下传至所有 `recordUsage` 调用点，写入 `usage.Entry.SessionKey`（`ring.go`，`json:"sessionKey,omitempty"`），`/usage` JSON 经结构体 tag 自动暴露（`getUsage` 无字段过滤）。控制台 `[reqID]` 日志行扩展为 `[reqID|sess:<key>]`（`forward.go::reqLogTag`，空 key 退化为 `[reqID]`），应用于 REQUEST/SEND/PROXY/no-available-keys/各 WARN 行。前端 `usage.js` Recent Requests 新增"按会话分组"开关。**推理非 ground truth**：截历史客户端致 root 漂移→会话分裂；并发同 root 会话（罕见）合并。v1 用简单 root hash，不做滑窗指纹。
>
...
>
> **2026-07-26 更新（Recent Requests 合并保活 + 详情 modal 回退 API 兜底，行为变更）：** 修复 2 个剩余症状——"大部分请求出现又消失"（Bug A）与"完成的请求详情 modal 无响应头/响应体"（Bug B）。**Bug A 根因：** `refreshQuotaData` 的 `lastUsageEntries = merged` 全量替换会在 `handleRequestDone` 已删除 `inflightEntries[id]` 且 ring 条目已被驱逐（或 `limit=500` 截断）时丢弃条目——条目既不在 API 响应中也不在 `inflightEntries` 中，永久消失。**修复：** `refreshQuotaData`（`usage.js:901-914`）在排序 `merged` 后，遍历 `lastUsageEntries`，将具有终态（status ≠ processing）且不在 `seenIds`（即不在 API 响应中）的条目追加回 `merged`，再重新排序赋值。**Bug B 根因：** `showUsageEntryInfoById`（`usage.js:1792`）从 `lastUsageEntries` 找到的条目可能是 processing 副本（无 respPayload/respHeaders）——当 `handleRequestDone` 因 SSE 事件丢失（broadcaster 缓冲满）退化为 `completeEntry = inflightEntry` 时，状态被设为 success/error 但 payload 字段缺失，或 `refreshQuotaData` 合并时仅保留了 tracker 的 processing 副本。**修复：** `showUsageEntryInfoById` 改为 async，当条目为 processing 状态且已不在 `inflightEntries` 中，或条目有终态但无 respPayload/respHeaders 时，通过 `apiGet('/usage?limit=100')` 从后端拉取 ring 条目替换。两个 bug 独立——Bug A 是合并丢失，Bug B 是 modal 查找指向错误条目。
...
| 修改用量/在途/兜底清理 | recorder.go（16-90）+ entry_tracker.go（13-138，含 `SetTTFT`/`UpdateTokens`/`SweepStale`）+ inflight.go（11-88）+ broadcaster.go（9-80）+ api/sse_events.go（16-79）+ forward_retry.go `broadcastTTFT`/`broadcastTokens` + stream.go token 广播 + 客户端断开补 recordUsage（`clientDisconnected` 标志 + 外层 `break` + 末尾 status 判断）+ api/usage.go `getUsage` 调用 `SweepStale(10min)` 转超时条目为 error + 前端 `usage.js`（`handleRequestStart` 去重、`handleRequestDone` 兜底清理、`updateProcessingLatencyCells` 超时停止、`refreshQuotaData` stale 判定）；改 `Entry.Source` 来源标记须同步前端 `X-TinyRouter-Source` 头（pg-stream.js）；改 `Entry.OriginalModel` 须同步 forward_request.go `handleProxy` alias 解析前捕获、所有 `recordUsage` 调用点、前端 `usage.js` `displayModelName`/`buildModelIdToAlias`/`renderUsageRow`；改 `QuotaBar.Alias` 须同步 `api/q…` |
| 修改 quota 聚合重算/配额单元格显示/ exhausted key 过滤 | **2026-07-31 新增**：`api/usage/register.go` `getQuotas` 从 per-key `ModelQuotas`（`GetQuota` 锁安全读取）重算 `TotalUsed`/`TotalCapacity`，覆盖 `QuotaTracker` 纯会话聚合；`web/static/usage_quota.js` `renderQuotaKeyRows` 跳过 `data.hasQuota && k.hasQuota && k.modelRemaining === 0` 的 key；`formatQuotaCell(bar)` 渲染 `success/capacity` + 可选 error badge；`renderQuotaRow`/`patchQuotaRow`（innerHTML）消费；`web/static/style.css` 新增 `.quota-success`/`.quota-capacity`/`.quota-error-badge` 类、列 4 宽度 110px。 |
