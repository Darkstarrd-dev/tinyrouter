...
> **最后核对（2026-08-08，Monitor Recent Requests 详情弹窗）：** `web/static/monitor/monitor_modal.js` 的详情弹窗固定六个 section：`Request Info`、`Request`、`Request Headers`、`Response Headers`、`Status`、`Response Body`。默认全部折叠；`Status` 仅保留 section 折叠，不提供 section 级 Pretty/Raw/Copy，其余五个 section 提供 section 级 Pretty/Raw/Copy；字段仍提供字段级 Pretty/Raw/Copy。Raw 必须保留捕获到的原始字符串，不得由 Pretty 结果重新序列化生成；弹窗采用 section header 与 field header 两级 sticky。`info_common.js` 的 `renderInfoSection`/`buildInfoField` 是共享兼容边界，Monitor 仅通过既有 section/field 能力扩展，不改变其他 info modal 调用方默认行为。
