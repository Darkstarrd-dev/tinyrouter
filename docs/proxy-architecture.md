...
> **最后核对：** 2026-08-04，仓库工作区（`main`）。**Provider 自定义请求头**：`config.Provider.UseCustomHeaders`/`CustomHeaders` 经 `internal/customheaders.Apply` 应用于正常代理、GET 任务轮询、Provider 管理探测/模型拉取、多协议/多 Key 探测和 Combo 测速；自定义头在现有认证、Content-Type、Accept 与客户端透传头之后应用，保持旧配置零值 no-op；Cline 的 `x-client-type: cline-cli` 仍最后覆盖同名自定义值。此前核对内容保留如下：**P0/P1/P4 修复**：`recorder.go` reqBody 截断后新增 `json.Valid` 校验防止存储破碎 JSON（对齐 respBody 逻辑）；`forward_retry.go` `processingEntry` 新增 `SessionKey` 字段使 in-flight / `request-start` 条目携带会话键（修复“按会话分组”在请求完成前不生效）；`handler.go` `RequestUpdates` Broadcaster 缓冲区由 64 扩至 256 防止高并发 SSE chunk 洪泛下丢弃 `request-done` 事件。**payload 捕获门控**（2026-07-28，详见 §11.1）：ring 存储 `captureDetails := isPlayground || (h.debugMode() && !h.logRequests())`——playground 始终捕获；非 playground 仅在 debug 开 且 追踪关 时存 payload/headers（追踪开时完整 body 已落盘 JSONL 由 Log Reader 查看，ring 退化为轻量表避免重复占内存；debug 关亦不存）。响应体累积 `captureDetails := h.logRequests() || isPlayground || h.debugMode()`（stream.go 两处），任一消费者（追踪/debug ring/playground）需要即累积，使追踪的响应体捕获独立于 debug。**两层 JSONL 追踪日志**（见 2026-07-27 更新条目）…
> **最后核对（2026-08-03）：** Quota Monitor 主行 latency/avg-speed 首次渲染与周期回填路径已按 `monitor_quota.js::refreshAllKeyDetails` 核对。
> **最后核对（2026-08-03，Monitor UI）：** `monitor_state.js` 的动态列宽与 `monitor_recent.js` 的 provider/model 搜索已按当前实现核对。
> **最后核对（2026-08-08，Monitor Recent Requests 详情弹窗）：** `web/static/monitor/monitor_modal.js` 的详情弹窗固定六个 section：`Request Info`、`Request`、`Request Headers`、`Response Headers`、`Status`、`Response Body`。除 `Status` 为无折叠、无 Pretty/Raw/Copy 的静态状态行外，其余五个 section 默认折叠并提供 section 级 Pretty/Raw/Copy；字段仍提供字段级 Pretty/Raw/Copy。section Raw 直接显示该 section 的原始文本，不经过字段解析；字段 Raw 直接显示字段原文；只有 Pretty 执行 JSON/Markdown/SSE 解析。弹窗采用 section header 与 field header 两级 sticky。`info_common.js` 的 `renderInfoSection`/`buildInfoField` 是共享兼容边界，Monitor 仅通过既有 section/field 能力扩展，不改变其他 info modal 调用方默认行为。
> **最后核对（2026-07-30 追踪日志目录可配置）：** `handler.go` 的 `requestLogDir` 由 `string` 改 `atomic.Value`，`SetRequestLogDir`/`TracesDir` 走原子读写以支持运行时经 Settings API 重指（`config/paths.go::ResolveTraceDir` 解析 `TraceConfig.LogDir`）；`request_log.go` 全部直读 `h.requestLogDir` 改走 `TracesDir()`。`api/trace/register.go::getDates` 对不存在的新目录（`os.IsNotExist`）返回空日期+目录而非 500，使 Editor Log Reader 在切换目录后显示空状态而非报错。
> **最后核对（2026-08-03，审计修复）：** (1) `passThroughResponse` 移除 `io.LimitReader(resp.Body, 64<<20)` 静默截断——完整读上游 body 并原样写客户端（客户端断开由写错误/请求 context 处理），仅 usage 捕获副本截 512KB（stream.go，回归测试 `TestPassThrough_LargeBodyStreamsFully`）；(2) 流式捕获缓冲 `sseBuf` 换 `boundedSSEBuffer`，只保留末尾 512KB（stream.go），长流不再撑爆内存；(3) `writeRequestLog` 落盘前经 `boundTraceBody` 截断 reqBody 64KB / respBody 512KB 并剥离 base64 图像（`request_log.go`，此前追踪写的是未截断全量 body）；`SweepTraces` 的 MaxDiskMB 上限现覆盖整个 traces/ 目录（含 `index-*.jsonl`），删 req 文件时同步清理 `attemptCounter`；删除死代码 `getAttemptCount`/`sanitizeFilename`/`formatBody`（及对应测试）；(4) 未映射 5xx 由 `ActionTransient`（30s 锁健康 key）改为 `ActionBackoff`（`error_rules.go` 新增 `{StatusMin:500,StatusMax:599}` 区间规则，`ErrorRule` 新增区间字段）；(5) `forwardWithRetry` 入口对 `parsed` 做深度拷贝（`forward.go::cloneJSONValue`）——combo 各目标共享同一 map 的逐目标改写（model/stream_options/tool_call id/Gemini 签名）不再跨目标泄漏；`processingEntry.ReqHeaders`/ring 的 `entry.ReqHeaders` 统一经 `maskHeaderMap` 遮蔽（Authorization/X-Api-Key 与追踪同策略）；(6) `forwardGetUpstream` 按 `UseProxy` 选择代理/直连客户端（`upstreamClientFor`/`streamClientFor` 抽取）；`string(body)` 拷贝改 `bytes.NewReader`；(7) `SetUpstreamTimeout` 改为原子 `upstreamTimeoutSec`，`clientFor` 按需克隆客户端（不再并发写 `http.Client.Timeout`）；(8) `generateRequestID` 折叠原子计数器进后缀（同纳秒并发不再碰撞）。
> **2026-07-29 更新（Usage 前端拆分与重构，行为不变）：** `web/static/monitor.js`（1634 行）拆为 6 文件，`index.html`/`index-nopg.html` 加载顺序 `monitor_state→monitor_io→monitor_quota→monitor_recent→monitor_modal→monitor.js`（`//go:embed all:static` 自动纳入）。**条目合并抽取**：`refreshQuotaData` 与 `renderUsage` 原各自的去重/inflight 合并/terminal 保留逻辑合并为 `monitor_io.js::mergeUsageEntries`（O(n) Map 索引替代 `.find()`，**行为不变**——§8 Recent Requests 去重/保活/会话分组描述仍准确）。**Quota Monitor 性能**：`monitor_quota.js::updateQuotaTable` 停止每周期 sub-row 重建，改条件性 DocumentFragment 整组重排（主行+其 `.quota-key-row` 兄弟一并迁移，稳态 0 次 DOM 移动，sub-row 不孤立）；`refreshAllKeyDetails` 为回填未展开主行的 latency/avg-speed 会请求所有 quota bar，cache 新鲜（TTL 内）的行跳过；展开仍由 `toggleQuotaRowExpand` 直接 `fetchModelKeyDetail`（绕过 TTL），sub-row 渲染仍由 `expandedModels` 门控。**i18n/清理**：`monitor_modal.js` Info Modal 硬编码英文走 `t()`（`i18n.js` 新增 `loadFailed`+17 `info*` 键）；删除死代码 `lastUsageSig`/`lastQuotaSig` + 4 个废弃 recent-requests-modal 函数 + `showUsageEntryInfo(ts)`。**文件锚点迁移**（供下方变更维护清单的 `monitor.js` 引用对照）：`mergeUsageEntries`/`refreshQuotaData`/`applyUsageSSEHandlers`/`handleRequestStart`/`handleRequestDone`/`handleRequestChunk`/`handleRequestTTFT`/`handleRequestTokens`→`monitor_io.js`；`renderUsageRow`/会话分组（`renderRecentRowsGrouped`/`toggleSessionGroup` 等）/`updateUsageSummary`→`monitor_recent.js`；`renderQuotaRow`/`patchQuotaRow`/`updateQuotaTable`/`toggleQuotaRowExpand`/`fetchModelKeyDetail`/`refreshAllKeyDetails`→`monitor_quota.js`；`showUsageEntryInfoById`/`showUsageEntryInfoWithData`/`loadTraceDetails`/`updateStreamingModalResponse`→`monitor_modal.js`；`displayModelName`/`buildModelIdToAlias`/`formatBody`/`sortEntriesByTimeDesc`→`monitor_state.js`；`renderUsage`/`updateProcessingLatencyCells`/`startUsageRefresh`/`stopUsageRefresh`/`ensureProcessingTimer`/`resetQuotaTimers`→`monitor.js`。
> **2026-08-03 更新（Quota Monitor latency/avg speed 修复）：** `monitor_quota.js::refreshAllKeyDetails` 必须为每个 quota bar 请求 `/monitor/model-keys`，因为顶层 latency/avg-speed 单元格初始为 `—`，且 `patchQuotaRowActiveMetrics` 只在 `fetchModelKeyDetail` 完成后回填。此前为优化展开行而跳过 collapsed rows，导致未展开模型永远不请求详情、两列永久空白；现由 `renderUsage` 在首次绘制后立即触发，周期刷新继续使用 `KEY_DETAIL_TTL`，并保留 `expandedModels` 门控仅用于 sub-row 渲染。
> **2026-08-03 更新（Quota Monitor 首次加载空白修复）：** `/api/monitor/quotas` 现在把 `EntryTracker` 中仍在处理的非 Playground 请求按 provider/model 加入 provisional quota bar。此前首次打开 Monitor 时请求已进入 Console/Recent Requests 但尚未完成，`QuotaTracker` 与累计 `ModelStats` 都尚未产生该模型，API 返回空数组；请求完成后才出现 quota 行。
> **2026-08-03 更新（Monitor 表格自适应与 Recent Requests 搜索）：** `web/static/monitor_state.js::fitMonitorTable` 在每次 Monitor 首次渲染、quota/recent 数据更新、窗口尺寸变化和 `data-font-size` 变更后，使用隐藏的 intrinsic-layout 克隆表格测量各列最长可见单元格，写入 `<colgroup>` 后以 `table-layout:fixed` 稳定表头与数据列；容器允许横向滚动，字号变化会重新测量。`monitor_recent.js` 在 Recent Requests 标题栏复用 Console 风格 search input，`onRecentSearch` 按 provider、原始/解析 model、alias 显示名进行不区分大小写筛选，分页和空结果提示同步更新且输入框不因 tbody 局部刷新失焦。`i18n.js` 同步中英文 placeholder/空结果文案。
> **2026-08-04 Monitor 前端样式整改：** `web/static/monitor_recent.js` 将 pager disabled、session indentation/visibility 等视觉状态迁移至 class；`monitor_quota.js` 将 quota clickable/expanded/chevron 状态迁移至 class；对应 CSS 由 `style.css` 语义 Token 层驱动，保留表格列宽等计算几何 inline 值。
> **2026-08-04 Monitor Recent Requests 分页修复：** `monitor_recent.js::updateRecentPagerState` 在更新上一页按钮前计算 `atFirst`，并与 `atLast` 一起按当前页/最大页同步 `disabled` 与 `pager-disabled` class；避免首次渲染和 SSE 刷新触发 `ReferenceError`，分页按钮状态保持与 `recentPage` 边界一致。
> **2026-08-05 Monitor 布局、Filter 分段聚合、Bottom-Line 检索框、自适应表头与 Chevron 行列重构：** `quota-monitor-card` card-title、`recent-requests-card` card-title 及 `console-toolbar` 统一设定 `padding: 10px 14px; min-height: 28px` 内边距与 `flex-wrap: nowrap` 单行防换行对齐；Filter 按钮采用 `.btn-filter-group` 容器实现紧密无间隙聚合（Segmented Control 样式）；检索框参照 Download 页面仅保留底部下划线（`border-bottom: 1px solid`）、提示文字居中对齐、支持弹性缩短与文本截断；`fitMonitorTable`（`monitor_state.js`）升级为双态自适应计算（容器充足时 100% 满宽铺满，受限时按实际内容横向滚动）；移除 Quota Monitor 表格独立占据一列的 chevron 列（从 8 列精简为 7 列），将折叠/展开指示图标内联前置于 Provider 单元格中，展开/折叠时附带平滑旋转动画，子行首列维持 22px 缩进层级；`.quota-section-scroll` 右侧边距设为 `padding-right: 0`。
> **2026-07-31 更新（Quota 单元格显示格式 + exhausted key 过滤）：** `monitor_quota.js`(a) `renderQuotaKeyRows` 跳过 `data.hasQuota && k.hasQuota && k.modelRemaining === 0` 的 key（已耗尽 key 不再显示在展开的 per-key 列表中，其配额贡献已计入 provider 级聚合）；(b) 新增 `formatQuotaCell(bar)` 辅助，渲染顶层配额单元格为 `<span class="quota-success">successCount</span> / <span class="quota-capacity">totalCapacity|∞</span>` 加可选红色 `<span class="quota-error-badge">errorCount</span>`（仅 errorCount>0）。`renderQuotaRow`/`patchQuotaRow`（现用 innerHTML）消费之。unlimited 模型显示 `successCount / ∞`。`style.css` 新增 `.quota-success`（绿 `--accent2`）、`.quota-sep`（淡化）、`.quota-capacity`（二级色）、`.quota-error-badge`（红 badge）类；列 4 宽度从 84px 扩至 110px。`getQuotas`（`internal/api/monitor/register.go`）现从 per-key `ModelQuotas`（`GetQuota` 锁安全读取）重算 `TotalUsed`/`TotalCapacity`，覆盖 `QuotaTracker` 的纯会话聚合，使重启后 `QuotaTracker` 为空时 exhausted key 的配额贡献仍计入 provider 级总量。
> **2026-08-02 更新（api.cline.bot 免费模型请求头注入，行为新增）：** `config/types.go` 新增 `Provider.IsCline()`（BaseURL 含 `api.cline.bot`，与 `IsNIM`/`IsGeminiOpenAICompat` 同模式）；`upstream.go` 新增 `applyClineHeaders`（122-128）+ 常量 `clineClientTypeHeaderValue="cline-cli"`（117-120），`forwardUpstream`（57-60）与 `forwardGetUpstream`（143）对 cline provider **无条件设置 `x-client-type: cline-cli`**（覆盖客户端原值）——`cline-free/*` 免费模型在 api.cline.bot 无此头会 403（`only available via Cline product surfaces`），付费模型（`deepseek/` 等）不依赖此头、带上无害。测试：`config_test.go::TestIsCline`、`handler_test.go::TestForwardUpstream_ClineClientTypeHeader`/`TestForwardGetUpstream_ClineClientTypeHeader`（captureTransport 拦截上游请求，无需真实网络）。
> **2026-07-31 更新（页面层 usage→monitor 更名，行为不变）：** Monitor 页面（导航显示名）对应的内部标识全部对齐：前端 `web/static/usage*.js` 6 文件更名 `monitor*.js`（`monitor_state/io/quota/recent/modal/monitor.js`，加载顺序不变），`data-page="usage"`→`"monitor"`、快捷键 `global.goto-usage`→`global.goto-monitor`、i18n 键 `usage`→`monitor`；后端 `internal/api/usage` 包更名 `internal/api/monitor`，管理路由 `/api/usage/*`→`/api/monitor/*`（含 SSE `/api/monitor/events`，`api/sse/register.go`）；数据层 `internal/usage`（`RingBuffer`/`Entry`/`QuotaTracker`、`recordUsage`、`stream_usage.go`、`config.UsageRingSize`）与 OpenAI 协议 `usage` 字段**保留原名**。前端函数名（`renderUsage` 等）与 `usage-updated`/`key-inflight` SSE 事件类型不变。注意：`internal/api/monitor` 与 2026-07-31 已删除的 terminal 监控 `internal/api/monitor` 同名异义（见 config-registry-state-architecture.md 增补#28）。
> **会话连续性推理（2026-07-26 新增）：** `forward.go::sessionKeyFromMessages` 从请求体 `messages` 的 system 消息 + 首条 user 消息内容（各截 4096 rune，null 分隔）算 FNV-1a 64→8 hex 指纹，作为推断的 per-session key（空=单发/未分组）。`handleProxy`（`forward_request.go`）计算后经 `forwardWithRetry`/`handleCombo`/`retry.go` handlers/`stream.go` 一路下传至所有 `recordUsage` 调用点，写入 `usage.Entry.SessionKey`（`ring.go`，`json:"sessionKey,omitempty"`），`/monitor` JSON 经结构体 tag 自动暴露（`getUsage` 无字段过滤）。控制台 `[reqID]` 日志行扩展为 `[reqID|sess:<key>]`（`forward.go::reqLogTag`，空 key 退化为 `[reqID]`），应用于 REQUEST/SEND/PROXY/no-available-keys/各 WARN 行。前端 `monitor_recent.js` Recent Requests 新增"按会话分组"开关。**推理非 ground truth**：截历史客户端致 root 漂移→会话分裂；并发同 root 会话（罕见）合并。v1 用简单 root hash，不做滑窗指纹。
>
...
> **2026-07-22 更新（Playground/Recent Requests 物理分流）：** `recorder.go`、`stream.go` 按 source 分流：`source == "playground"` 的请求写入独立 `pgUsageBuf`（经 `Handler.SetPgUsage` 注入），始终捕获 payload/headers；其余请求写入 `usageBuf`，仅在 `debugMode()` 时捕获 payload/headers。`reqBody` 截断上限 64KB、`respBody` 截断 512KB。`stream.go` 的 `sseBuf` 累积和 `passThroughResponse` 的 bodyBytes 仅在需要详情时参与。`api/monitor/register.go` 新增 `getPlaygroundUsage`（`GET /api/monitor/playground`），前端 `pgFetchReqLeft` 改用该端点。Recent Requests 的 `getUsage` 过滤掉 playground 来源的 inflight 条目。
> **2026-07-26 更新（payload 捕获与 debugMode 解耦，行为变更）：** 修复"Recent Requests 在 debug 模式切换后记录不可查看/旧记录变裸点"问题。`recorder.go`、`stream.go`、`passThroughResponse` 中的 `captureDetails` 现恒为 `true`——`ReqPayload`/`RespPayload`/`RespHeaders`/`ReqStatus`/`ReqHeaders`/`UpstreamURL` 对所有来源（含非 playground）**始终捕获**，使 Recent Requests 在任意 debugMode 状态下都可查看。内存开销由 ring 容量（`config.UsageRingSize`，默认 500）× 单条 body 大小封顶（reqBody 截断 64KB、respBody 512KB），与原 debug=on 行为一致。`debugMode()` 的其余门控保留：`stream.go` 中 `parseAndBroadcastChunk`（SSE debug 控制台实时推理面板广播）仍由 `h.debugMode()` 门控。前端 `monitor_modal.js` info 按钮的 `usageDebugMode` 门控移除，改为只要有 payload 数据即点击可查（修复后新记录恒有点击能力；修复前在 debug=off 时记录的旧记录因 payload 从未存储，仍为裸点，无法回溯恢复）。**（2026-07-28 撤销非 playground 部分；2026-08-03 更正为精确公式：`captureDetails := isPlayground || (h.debugMode() && !h.logRequests())`——playground 始终捕获，非 playground 仅 debug 开 **且追踪关** 时捕获（追踪开时完整 body 已落盘 JSONL，ring 退化为轻量表）。详见 §11.1。）**
> **2026-07-26 更新（会话连续性推理，行为新增）：** OpenAI 协议无 session ID；LLM IDE 客户端每轮重发完整 `messages`（msgCount 单调增长，root 稳定），故可推断会话归属。新增 `forward.go::sessionKeyFromMessages`：取 `messages` 中首条 `role=="system"` 与首条 `role=="user"` 的 `content`（string 或 `[{type:"text",text:...}]` 数组——`extractMessageContent` 拼接 text 字段），各 `truncateRunes` 截 4096 rune 后以 `\x00` 分隔拼接，`hash/fnv` FNV-1a 64→`%016x`[:8]；无 user 消息→`""`。`recordUsage` 新增 `sessionKey string` 参数写 `Entry.SessionKey`；`forwardWithRetry`/`handleCombo`/`logRequest`/`handleNetworkError`/`handle429`/`handleUpstreamError`/`streamResponse`/`passThroughResponse` 均新增该参数下传。控制台 `[reqID]` 标签经 `forward.go::reqLogTag` 扩展为 `[reqID|sess:<8hex>]`（空 key 时仅 `[reqID]`，不显示 `sess:`）。测试：`forward_session_test.go`（算法 6 类用例）+ `forward_session_e2e_test.go`（端到端：Entry.SessionKey 非空 + 控制台含 `|sess:` + 同 root 跨 turn 同 key + 不同首条 user 不同 key + 无 user→空 key 且无 `|sess:`）。前端 `web/static/monitor_recent.js` Recent Requests 新增"按会话分组"开关（`recentGroupBySession` + `renderRecentRows`/`renderRecentRowsGrouped`/`renderSessionGroupHeader`/`toggleRecentGroupBySession`/`toggleSessionGroup`），分页先于分组（对当前页切片分组，空 key 入"未分组"伪组置顶），`i18n.js` 新增 `groupBySession`/`ungrouped` 键。
...
> **2026-07-26 更新（Recent Requests 前端合并去重 + inflightEntries 清理，行为变更）：** 修复 4 个关联症状——已完成请求 Latency 继续增长（计时不停止）、Output Tokens 显示 0、详情 modal 无响应头/响应体、"按会话分组"折叠失效（同 sessionKey 的请求显示为平铺行）。根因：`monitor_io.js` 的 `refreshQuotaData` 与 `monitor.js` 的 `renderUsage` 周期合并逻辑中，`newEntries.map()` 对 API 返回的 `combined = ringEntries + inflightEntries` 不做 ID 去重——当 ring（已完成条目，status=success/error，携带 outputTokens/respPayload/respHeaders/sessionKey）与 EntryTracker（在途条目，status=processing，无上述字段）因竞态/泄露窗口对同一 request ID 同时存在时，两条目均进入 `lastUsageEntries`。processing 条目无 sessionKey → 落入"未分组"伪组单独显示；info modal 的 `find()` 返回首条（若排序后 processing 在前则读到空字段）。修复：合并循环改为 `forEach` + `seenIds` 去重（首次出现保留，后续同 ID 跳过——ring 条目在 API 响应中先于 inflight 条目，故完成条目天然胜出）；合并后新增扫尾：对 `merged` 中终态条目（status ≠ processing）删 `inflightEntries[id]`，防止其随 ring 条目被逐出后在后续轮询重新 `unshift` 注入。前端纯 JS 修改，后端未变。
>
> **2026-07-26 更新（Recent Requests 合并保活 + 详情 modal 回退 API 兜底，行为变更）：** 修复 2 个剩余症状——"大部分请求出现又消失"（Bug A）与"完成的请求详情 modal 无响应头/响应体"（Bug B）。**Bug A 根因：** `refreshQuotaData` 的 `lastUsageEntries = merged` 全量替换会在 `handleRequestDone` 已删除 `inflightEntries[id]` 且 ring 条目已被驱逐（或 `limit=500` 截断）时丢弃条目——条目既不在 API 响应中也不在 `inflightEntries` 中，永久消失。**修复：** `refreshQuotaData`（`monitor_io.js`）在排序 `merged` 后，遍历 `lastUsageEntries`，将具有终态（status ≠ processing）且不在 `seenIds`（即不在 API 响应中）的条目追加回 `merged`，再重新排序赋值。**Bug B 根因：** `showUsageEntryInfoById`（`monitor_modal.js`）从 `lastUsageEntries` 找到的条目可能是 processing 副本（无 respPayload/respHeaders）——当 `handleRequestDone` 因 SSE 事件丢失（broadcaster 缓冲满）退化为 `completeEntry = inflightEntry` 时，状态被设为 success/error 但 payload 字段缺失，或 `refreshQuotaData` 合并时仅保留了 tracker 的 processing 副本。**修复：** `showUsageEntryInfoById` 改为 async，当条目为 processing 状态且已不在 `inflightEntries` 中，或条目有终态但无 respPayload/respHeaders 时，通过 `apiGet('/monitor?limit=100')` 从后端拉取 ring 条目替换。两个 bug 独立——Bug A 是合并丢失，Bug B 是 modal 查找指向错误条目。

> **2026-07-26 更新（Recent Requests "按会话分组" UI 修复，行为变更）：** 修复 3 个关联症状——(1) 会话默认折叠、展开后很快自动回折；(2) 折叠态下因 session 编号长度不一，session 名/请求数/时间/provider/model 各字段挤在一起未对齐；(3) 展开会话时带动 `<thead>` 表头文字移位。**根因（综合）：** `renderRecentRowsGrouped` 恒以 `hidden=true` 渲染子行且无展开态记忆，周期刷新 `updateRecentRequestsInline` 重渲染 tbody 即把已展开会话重置为折叠（症状 1）；`renderSessionGroupHeader` 用单个 `colspan=7` 单元格 + `·` 分隔的纯文本流，字段宽度随内容自适应（症状 2）；`.usage-table` 为 `table-layout:auto`，列宽随可见行内容重算，会话展开/折叠改变可见行集合即导致列宽漂移、表头随之移位（症状 3）。**修复：** (a) 新增模块级 `expandedSessions`（sessionKey→true）记忆展开态，`toggleSessionGroup` 改为以该集合为唯一真相源增删并同步 `collapsed`/`expanded` class 与箭头，`renderRecentRowsGrouped`/`renderSessionGroupHeader` 据其决定子行 `hidden` 与 header class/箭头，`toggleRecentGroupBySession` 关闭分组时清空集合——周期重渲染不再折叠已展开会话。(b) `renderSessionGroupHeader` 改为 `<td colspan=7 class=session-group-cell>` 内嵌 `<div class=session-group-flex>` 的 flex 布局，`session-group-name`(flex:0 0 120px)/`-count`(0 0 90px)/`-time`(0 0 180px)/`-model`(1 1 auto,ellipsis) 各段预分配固定宽度，不再随 session 编号长度挤压。(c) `.usage-table` 改 `table-layout:fixed` 并以 `th:nth-child(n)` 固定 7 列宽度（28/76/88/160/120/68/88px），`td[colspan]{max-width:none}` 放开会话头单元格——列宽不再随可见行内容重算，会话展开/折叠时表头恒定不移位。前端纯 JS/CSS，后端未变。
...
- **注册方式：** 仅注册 `POST` 方法（`r.Post("/v1/embeddings", proxyHandler.Embeddings)`，router.go:232）。CORS 由 §3 的 `/v1/*` OPTIONS 处理自动覆盖。
...
> **2026-07-25 更新：** `BuildUpstreamURL`、`normalizeBaseURL`、`isOllamaBaseURL`/`normalizeOllamaBaseURL`、`isHostRoot` 已从 `internal/proxy/upstream.go` 提取到 `internal/urlutil/urlutil.go`。`proxy` 与 `api` 均导入 `internal/urlutil` 使用。`proxy/upstream.go` 中的 `forwardUpstream`/`buildUpstreamRequest`/`forwardGetUpstream` 调用 `urlutil.BuildUpstreamURL`。`api` 中的探测/测速/管理代码同样调用 `urlutil.BuildUpstreamURL`。

`internal/urlutil` 提供：
- `isOllamaBaseURL` / `normalizeOllamaBaseURL`（upstream.go:17-46）：Ollama 特例。`isOllamaBaseURL` 纯字符串匹配（无网络探测，本地服务可能未运行）：host 为 `ollama.com`，或 host 为 `localhost`/`127.0.0.1` 且端口为 `11434` 时判定为 Ollama。`normalizeOllamaBaseURL` 把整个 path/query/fragment 清空，把 BaseURL 降为 host root。设计动机：Ollama 同时提供原生 `/api/*`（非 OpenAI 格式，响应/SSE/工具调用结构均不同，需完整格式转换）与 OpenAI 兼容 `/v1/*`（标准 OpenAI 格式，零转换）；本项目"不做格式转换、SSE 原样透传"原则决定了必须走 `/v1/*`。无论用户填什么路径（原生形如 `/api`、`/api/tags`、`/api/chat`，或 OpenAI 形如 `/v1`、`/v1/chat/completions`），清空 path 后由 host-root 分支统一注入 `/v1`。
...
- 新算法彻底解决了原 bug：
  ...
  - bug C（Ollama）：`BuildUpstreamURL("https://ollama.com/api", "/v1/chat/completions")` → `isOllamaBaseURL` 为真 → 清空 path → `https://ollama.com` → host-root 注入 `/v1` → `https://ollama.com/v1/chat/completions`（不再落到 404 的 `/api/*`）。本地 `http://localhost:11434/api` 同理归一为 `http://localhost:11434/v1/chat/completions`。
...
1. **URL 统一由 `BuildUpstreamURL` 构造：** Anthropic 上游 URL 不再单独实现 raw/完整 endpoint/host-root 三分支，而是改为一行调用 `BuildUpstreamURL(sel.Provider.BaseURL, "/v1/messages")`（upstream.go:85）。启发式 A 自动判断 BaseURL 是否含版本段：若已含 `/v1`（如 `https://api.anthropic.com/v1` 或 `https://api.anthropic.com/v1/messages`）则不注入额外 `/v1`；若为 host root（如 `https://api.anthropic.com`）则注入 `/v1`。推荐配置形如 `https://api.anthropic.com` 或 `https://api.anthropic.com/v1` 或 `https://api.anthropic.com/v1/messages`。
...
非流式成功响应：设置 `Content-Type: application/json` 与 `X-TinyRouter-*` 头，用 `w.WriteHeader(resp.StatusCode)` **原样透传上游状态码**（stream.go:312-317），**完整读取上游 body 并原样写客户端**（2026-08-03 起移除 64 MiB `io.LimitReader` 静默截断；客户端断开由写错误/请求 context 处理），仅 usage 捕获副本截 512KB，写出后 `util.ExtractTokens` 提取用量并 `recordUsage`（stream.go:324-340）。`io.ReadAll` 错误路径补 `recordUsage(status="error")`（stream.go:405-408）。客户端断开统一为 `status="error"`、`errMsg="client disconnected"`（stream.go:334-341）。

### 8.7 非流式 keep-alive 延迟刷新（已移除 / H-8 修复）

> **2026-07-26：该机制已整体移除。** 原实现位于 `forwardWithRetry`（`forward_retry.go`），对 `!isStream` 请求启动后台 goroutine，等待 20s 宽限期（`keepAliveDelay`）后才提交 HTTP 200 头并写首字节 `"\n"`，随后每 5s（`keepAliveInterval` ticker）写 `" "` + `Flush`，经 `keepAliveDone`/`keepAliveStopped` channel 同步退出，`state.headersFlushed` 标记头是否已提交，`passThroughResponse` 据 `headersFlushed` 跳过 `WriteHeader`。
...
- **调用点：** 成功流式（stream.go:372）、成功非流式（stream.go:416）、网络错误（retry.go:57）、429（retry.go:79、109、121、134、153、161、174、187、194、201、212、221、235）、上游错误（retry.go:251）。
...
14. **recordUsage 错误状态粗粒度：** `status` 仅 `error`/`success` 两档（`client_disconnected` 合并为 `error`+errMsg），HTTP 状态通过 `RespStatus` 字段附带，不再细分细粒度错误类型（recorder.go:16-66）。
...
- `internal/proxy/stream.go`：SSELineBuffer（15-41）、SSEDataPayloads（50-64）、normalizeSSEChunk（74-110）、streamResponse（139-358，`entryFormat` 控制 OpenAI 专用 `parseAndBroadcastChunk` 仅 OpenAI 入口调用、anthropic 入口走 `parseAnthropicSSEUsage` 提取 usage；2026-07-21 新增 `contentCharsTotal` 累积器 line 181、token 进度广播 line 286-294；`originalModel` 透传到 recordUsage line 357）、passThroughResponse（266-312，H-8 后恒设头 + `WriteHeader(resp.StatusCode)`，`headersFlushed` 参数已移除，recordUsage line 306）、parseAndBroadcastChunk（399-418）、chunkDelta/parseSSEChunkDelta（427-512）、extractThoughtSignature（540-587）、parseAnthropicSSEUsage（441-470，读 message_start/message_delta 的 input/output tokens）。
- `internal/proxy/retry.go`：retryState（14-21）、maxRetries（33-39）、logRequest（42-67，含 `reqID`+`callerTag` 参数 + alias 优先显示）、handleNetworkError（70-78，`[reqID]` + 中文网络错误 WARN）、handle429（80-256，`originalModel` 参数）、handleUpstreamError（258-362，**返回 `bool`**：true=4xx `ActionPassThrough` 已原样写客户端+停止重试，false=继续切 key；pass-through 分支转发上游原始 `resp.StatusCode`+body，不锁/不排除 key；其他动作新增中文后果 WARN）、classifySenseNova429、excludeSameAccountKeys。
...
- `internal/proxy/entry_tracker.go`：EntryTracker（13-138，含 `SetTTFT` 74-82 / `UpdateTokens` 84-98，2026-07-21 新增；`SweepStale` 116-138，2026-07-23 新增）、MarshalEntryJSON（102-108）。
...
- `internal/proxy/request_log.go`：`writeRequestLog`（两层 JSONL 写入：`traces/index-YYYYMMDD.jsonl` 每日轮转 + `traces/req/<reqID>.jsonl` 追加）、`traceLine` 结构体（JSONL 行 schema）、`TraceMgmtCall` 方法（ManagementClient 路径探测捕获）、`SweepTraces`/`sweepTracesOnce`（后台保留清理）、`TracesDir() string` getter；
- `internal/api/trace/register.go`：3 个 GET 路由（`/api/traces/dates`、`/api/traces/index`、`/api/traces/req/{reqID}`），auth-gated，路径遍历防护（`sanitizePathParam` 拒绝 `/`、`\`、`..`、null 字节）。
