...
| `providers.go` / `providers_validate.go` / `providers_models.go` / `providers_models_crud.go` | Provider 列表/校验/模型拉取/模型增删改查（`addProviderModel`、`updateModelQuota`、`updateModelAlias`、`updateModelNote`、`updateModelNIM`、`deleteProviderModel`） |
...
| 修改 Playground 图片功能 | playground | `pg-modal.js`（`pgShowImageModal`/`pgInitImageZoom`）、`pg-render.js`（`pgMsgInnerHTML` 气泡 onclick）、`api/image.go`（save-image 端点）、`playground.css`（`.pg-img-btn`） |
