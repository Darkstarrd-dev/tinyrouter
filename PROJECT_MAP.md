...
| 修改 Image 模式端点/协议选择器/GPT 参数 | playground | `web/playground/static-pg/pg-core.js`（图片参数 + `imgComfyPort`/`imgComfyTemplateId`/`imgComfyWorkflow` 默认值），`pg-ui.js`（GPT/xAI/ModelScope/comfyui 协议选择、ComfyUI 连接面板与动态控件），`pg-request.js`（OpenAI 图片 body），`pg-stream.js`（现有 OpenAI 图片发送），`pg-comfyui.js`（ComfyUI `/system_stats`/`models`/`object_info`/`prompt`/`history`/`view` 调用与历史轮询；`/userdata?dir=workflows` 已保存 workflow 作为 `Tab Select` 候选，UI graph→API prompt 转换；saved + history 候选按 ComfyUI workflow `id` 聚合同一 Tab，无 id 时按规范化签名去重，saved 同 id/签名优先；当前 ComfyUI 页面 active Tab 需额外桥接/CDP，REST 代理无法直接读取），`pg-state.js`（Tab 候选运行态结构），`pg-i18n.js`（Tab Select 文案）、`playground.css`、`web/static/index.html`、`internal/api/comfyui/register.go`、`internal/api/router.go`（同源代理与静态白名单）` |
