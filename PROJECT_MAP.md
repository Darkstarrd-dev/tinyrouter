...
| JS 模块 | `app.js`、`api.js`、`auth.js`、`i18n.js`、`theme.js`、`info_common.js`、`providers.js`、`combos.js`、`quickslots.js`、`headerStats.js`、Monitor 拆分模块（`web/static/monitor/`）、`console.js`、`download.js`、`filetransfer.js`（Settings FileTransfer modal：任意文件拖拽/粘贴、Clear 重置、上传进度与确认上传）、Settings 拆分模块（`web/static/settings/`）、`gallery/editor` 入口依赖、`gif-editor/` 子目录：`gif-editor-state.js` / `gif-editor-import.js` / `gif-editor-timeline.js` / `gif-editor-playback.js` / `gif-editor-export.js` / `gif-editor.js`（GIF 帧编辑器 SPA 页面模块与 5 个子模块：`renderGifEditor(container)`/`cleanupGifEditor()`，在 `download.js` 之后按 state -> import -> timeline -> playback -> export -> editor 顺序加载，`navigateTo` switch `case 'gif'` 分发，`app.js` cleanup 段回收。**模块职责**：`gif-editor-state.js`=`window.GifEditorCore`（constants/state/dom/modules/commands/cleanupFns 注册表 + `byId`/`freshId`/`formatDurationMs`/`registerModule`/`registerCleanup`/`cleanupModules`/`resetSlices`）；`gif-editor-import.js`=`openFromFile`/`bindEvents`/`cancel`/`cleanup`（Import Modal：比例/FPS/双手柄起止/实际帧数与时长联动，commit 事务保留旧工程）；`gif-editor-timeline.js`=`render`/`updateWindow`/`ensureVisible`/`setZoom`/`getZoom`/`clearThumbCache`/`bindEvents`/`cleanup`（虚拟化窗口 + 50%–200% 倍率 + 复制/删除/拖拽排序/delay 输入）；`gif-editor-playback.js`=`first`/`previous`/`play`/`pause`/`toggle`/`next`/`last`/`updateButtons`/`bindEvents`/`cleanup` + 注册 `core.commands.focusFrame`（generation 守卫播放状态机 + ←/→/PgUp/PgDn/Home/End/Space 键盘）；`gif-editor-export.js`=`gif`/`zip`/`sprite`/`closeResult`/`openResultInGallery`/`bindEvents`/`cleanup`（三条导出路径 + MediaBridge 登记 + 结果弹窗）；`gif-editor.js`=`renderGifEditor`/`cleanupGifEditor`/`window.GifEditor` + 页面模板/cacheDom/`registerCoreCommands`（`redrawSelection`/`updateSelectionUI`/`composeFrame`/`resetView`）/`bindEvents`/画布绘制（`draw`/`drawLayers`/`composeFrame`/pan-zoom））、`media-bridge.js`（**2026-08-06 新增**，MediaBridge 交接契约：`register`/`openGallery`/`consume`/`deliverPendingImports`/`getAssetBlob`/`archiveStatus`；两个 index 变体均最先加载；配套 `web/media-bridge.test.js`（embed 目录外的 Node 冒烟测试）） |
...
| 类别 | 内容 |
|---|---|
| JS 加载顺序 | `playground/` 子目录：`pg-i18n.js` → `pg-core.js` → `pg-state.js` → `pg-markdown.js` → `pg-request.js` → `pg-stream.js` → `pg-comfyui.js` → `pg-image-model.js` → `pg-image-inspire.js` → `pg-image-batch.js` → `pg-autochat.js` → `pg-setup.js` → `pg-director.js` → `pg-search.js` → `pg-render.js` → `pg-ui.js` → `pg-modal.js` → `pg-lifecycle.js`，随后 `gallery/`、`editor/` 子目录（Gallery、Editor、Text Review）脚本 |
...
| 其他模块 | `playground/`：`pg-core.js`、`pg-state.js`、`pg-request.js`、`pg-stream.js`、`pg-comfyui.js`（每窗口 Comfy runtime、协议回退、按名称去重 Tab Select）、`pg-render.js`、`pg-ui.js`（Image 多窗口独立设置与 pane 选择）、`pg-modal.js`、`pg-lifecycle.js`、`pg-autochat.js`、`pg-setup.js`、`pg-director.js`、`pg-search.js`；`gallery/`、`editor/` 子目录文件 |
...
| 静态路由 | `static-pg` 按文件路由由 `internal/feature/feature.go` 的 `StaticFiles` manifest 经 `feature.Assets(RootPlaygroundPG)` 派生（`internal/api/router.go`），无硬编码 `pgJSFiles` 列表；URL 路径 = 子目录相对路径（`/playground/pg-core.js`、`/gallery/gallery.js`、`/editor/editor.js`） |
---
...
| 修改用量统计/在途跟踪/兜底清理 | proxy、config-registry-state | `proxy/recorder.go`、`entry_tracker.go`、`inflight.go`、`broadcaster.go`、`proxy/forward_request.go`、`proxy/forward_retry.go`、`proxy/forward_combo.go`、`proxy/retry.go`、`proxy/stream.go`（usage/inflight/sessionKey/token broadcast）；`internal/api/monitor/register.go`（usage/quota API）；`web/static/monitor/monitor_state.js`（Recent provider/model predicate；quota/recent 表格按最长可见内容测量列宽，并在字号/窗口变化时重算）、`web/static/monitor/monitor_io.js`（SSE、merge/refresh）、`web/static/monitor/monitor_quota.js`（Quota Monitor 表格/per-key 详情）、`web/static/monitor/monitor_recent.js`（Recent Requests 状态筛选、会话分组、分页、provider/model 搜索）`
...
| CSS/HTML 样式移植与验证 | css_implement_tips.md、PROJECT_MAP §18.2、DESIGN.md、css_upgrade_plan.md | `css_implement_tips.md`（参考代码拆解、TinyRouter shell/theme/embed 约束、结构→效果流程、视觉验证清单与失败模式）、`css_upgrade_plan.md`（CSS 优化计划：覆盖 CSS 基线、Token、cascade、共享控件、响应式/可访问性与条件式拆分）。2026-08-07 已完成三批高置信度语义 Token 清理，详细范围与未执行 HTTP/浏览器验证记录在 css_upgrade_plan.md §1.5/§1.6/§1.8。 |
...
| 修改文本审核批处理/节点参数 | proxy、playground | `internal/config/types.go`（`TextReviewNode.IntervalSec/BatchChars`）、`internal/config/defaults.go`、`internal/textreview/scheduler.go`（`acquireAndClaim` IntervalSec 门控+`dequeueBatch` 算法+`runBatch` 批处理）、`internal/textreview/session.go`（`RangeStart/RangeEnd`）、`web/playground/static-pg/editor/editor_textreview_step3.js`（Settings modal 加 intervalSec/batchChars/篇章范围输入+StartClean 后隐藏配置区+紧凑卡片+右栏内容选中）、`web/playground/static-pg/editor/editor.js`（`.ed-review-spacer`→`.ed-review-content`）、`web/playground/static-pg/editor/editor_textreview_state.js`（`rangeStart/rangeEnd`）、`web/playground/static-pg/playground/pg-i18n.js`（新键）、`web/playground/static-pg/playground.css`
