...
| 修改用量统计/在途跟踪 | proxy | `proxy/recorder.go`+`entry_tracker.go`（`SetTTFT`/`UpdateTokens`）+`inflight.go`+`broadcaster.go`、`proxy/forward.go`（`broadcastTTFT`/`broadcastTokens`，`processingEntry.InputTokens` 粗估）、`proxy/stream.go`（`contentCharsTotal` 累积 + token 进度广播）、`api/sse_events.go`、`usage/`、`web/static/usage.js`（`handleRequestTTFT`/`handleRequestTokens` + 筛选 Tag） |
