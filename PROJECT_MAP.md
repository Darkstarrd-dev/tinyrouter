...
| 修改 QuickSlot 头部交互 | PROJECT_MAP §18.2 | `web/static/quickslots.js`（`openQuickSlotModalByOrder`/`openQuickSlotModalById`/`_qsModal*` modal 系统 + capture 阶段键盘处理 + 1s 自动关闭门限 + Del 删除）、`web/static/app.js`（1-9 改调 `openQuickSlotModalByOrder(n, true)` + `isQuickSlotModalOpen()` 前置跳过）、`web/static/style.css`（`.qs-modal-overlay`/`.qs-modal`/`.qs-modal-item` 样式）、`web/static/i18n.js`（`qsModalHint`） |
