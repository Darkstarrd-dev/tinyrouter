...
| JS 模块 | `app.js`、`api.js`、`auth.js`、`i18n.js`、`theme.js`、`info_common.js`、`providers.js`、`combos.js`、`quickslots.js`、`headerStats.js`、Monitor 拆分模块（`web/static/monitor/`）、`console.js`、`download.js`、`filetransfer.js`（Utility FileTransfer：任意文件拖拽/粘贴、Clear、上传进度与确认上传）、Settings 拆分模块（`web/static/settings/`）、`utility/editor/`（File Editor：`editor-state.js`/`editor.js`；独立 Log Reader：`editor-logs.js`；独立 Text Review：`editor_textreview_*` + `review.js`）、`gif-editor/`（GIF 页面 state/import/timeline/playback/export/editor） |
