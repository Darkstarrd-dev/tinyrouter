...
| 修改 Image 模式端点/协议选择器/GPT 参数 | playground | `web/playground/static-pg/pg-core.js`（图片参数 + `imgComfyPort`/`imgComfyTemplateId`/`imgComfyWorkflow` 默认值），`pg-ui.js`（GPT/xAI/ModelScope/comfyui 协议选择、ComfyUI 连接面板与动态控件），`pg-request.js`（OpenAI 图片 body），`pg-stream.js`（现有 OpenAI 图片发送），`pg-comfyui.js`（ComfyUI `/system_stats`/`models`/`object_info`/`prompt`/`history`/`view` 调用与历史轮询；2026-08-05 起另经 `/userdata?dir=workflows` 导入 `user/default/workflows/*.json` 已保存工作流——`pgComfyGraphToPrompt` 将 UI graph 转 API prompt：widget 顺序对齐（cag 槽位 + `/object_info` schema 白名单）、Reroute 链回溯、subgraph 递归展开，模板按 saved(modified 倒序,`file:` 前缀)→history 合并），`pg-i18n.js`、`playground.css`、`web/static/index.html`、`internal/api/comfyui/register.go`、`internal/api/router.go`（同源代理与静态白名单）` |
...
| 修改 Playground 模式选择器样式 | playground、PROJECT_MAP §18.3、DESIGN.md | `web/playground/static-pg/pg-ui.js`（`pgState.mode`/`pgSetMode()` 业务状态不变）、`web/playground/static-pg/playground.css`（`.pg-mode-toggle` 无外部留白的凹陷分段按钮、active/hover/focus 状态）、`web/static/style.css`（`--pg-mode-*` dark/light Token）、`docs/playground-architecture.md` |
