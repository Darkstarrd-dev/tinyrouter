...
| 修改 Header 页面切换按钮样式 | PROJECT_MAP §18.2、DESIGN.md | `web/static/index.html`/`index-nopg.html`（可访问 nav shell + no-playground minimal layout）、`web/static/style.css`（3×2 reference grid、中心装饰方块、长条矩形按钮、`--nav-*` dark/light tokens；采用 `color-mix(in srgb, ...)` 算式绑定 18 种 Theme Variants，5 页面独立专属色彩方案；亮色模式精细化去除文字模糊扩散、发光伪元素与菱形染色） |
