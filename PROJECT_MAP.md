...
> **最后核对（2026-08-08，Utility 前端重组与 FileTransfer 上传路由）：** 当前管理 UI 将顶层 Download/GIF 收纳到 **Utility** 菜单；Utility active header label 显示当前工具，Utility 子工具为 `editor`、`logReader`、`review`、`gif`、`download`、`fileTransfer`；fresh init 只显示 Utility landing，不预选工具。Editor/Log Reader/Text Review 资产均位于 `web/static/utility/editor/`，仍属 `RootStatic`（`web/static`，所有构建嵌入），不再从 `web/playground/static-pg/editor` 提供。`web/static/index.html` 与 `index-nopg.html` 均加载 `/utility/editor/*` 入口脚本，其中新增独立 Utility Review wrapper `review.js`。Editor 仅为 File Editor，提供 Edit/Diff 双 pane raw/parsed 视图、sanitized Markdown preview、toolbar/status/find/diff；Log Reader 由独立 `logReader` 工具提供，Text Review 由独立 `review` 工具/wrapper 提供。Editor 使用 `web/static/vendor/utility-editor/` 的 `diff-match-patch`、`markdown-it`、`dompurify`、`turndown`、`prism` 及各自许可证文件；这是官方 StackEdit 的局部移植边界，不是完整 StackEdit/cledit/PageDown 实现，远程服务仍未移植。导航快捷键为 F5 打开 Utility 菜单、F4 直达 Gallery；无旧的 Gallery↔Editor toggle。Download/FileTransfer 的后端 API 路径未变；FileTransfer 当前注册 `POST /api/filetransfer/upload` 与 `POST /api/filetransfer/path-info`，并保留生命周期 hooks。
> **最后核对（2026-08-08，Monitor Recent Requests 详情弹窗）：** `web/static/monitor/monitor_modal.js` 固定六个 section：`Request Info`、`Request`、`Request Headers`、`Response Headers`、`Status`、`Response Body`；默认全部折叠。`Status` 仅提供 section 折叠，其余五个 section 提供 section 级 Pretty/Raw/Copy，字段级仍提供 Pretty/Raw/Copy；Raw 保留捕获原始字符串，不由 Pretty 结果重新序列化；section header 与 field header 两级 sticky。共享 `web/static/info_common.js` 的 `renderInfoSection`/`buildInfoField` 是兼容边界，Monitor 扩展不改变其他 info modal 调用方默认语义。
...
| JS 模块 | `app.js`、`api.js`、`auth.js`、`i18n.js`、`theme.js`、`info_common.js`（共享 info modal 的 section/field Pretty/Raw/Copy 与 raw-preservation 兼容边界）、`providers.js`、`combos.js`、`quickslots.js`、`headerStats.js`、Monitor 拆分模块（`web/static/monitor/`，其中 `monitor_modal.js` 固定六个 Recent Requests 详情 section，默认折叠、Status 仅折叠、其余 section 级与字段级 Pretty/Raw/Copy、两级 sticky header）、`console.js`、`download.js`、`filetransfer.js`（Utility FileTransfer：任意文件拖拽/粘贴、Clear、上传进度与确认上传）、Settings 拆分模块（`web/static/settings/`）、`utility/editor/`（File Editor：`editor-state.js`/`editor.js`；独立 Log Reader：`editor-logs.js`；独立 Text Review：`editor_textreview_*` + `review.js`）、`gif-editor/`（GIF 页面 state/import/timeline/playback/export/editor）`
| vendor | `vendor/gif.js/`、`vendor/gifuct-js/`（各含 LICENSE）；`vendor/utility-editor/`（`diff-match-patch`、`markdown-it`、`dompurify`、`turndown`、`prism`，各含 LICENSE）。Editor 仅实现官方 StackEdit 的局部 raw/parsed Markdown preview 与 sanitization 边界，不是完整 StackEdit/cledit/PageDown，也未移植 StackEdit 远程服务。 |
| 样式 | `style.css` |
...
>
> **当前 Utility 页面边界：** Editor 是 File Editor only：双 pane raw/parsed、sanitized Markdown preview，以及 toolbar/status/find/diff；其工具栏只有 Edit/Diff。Log Reader 以独立 `logReader` Utility tool 渲染，Text Review 以独立 `review` Utility tool 及 `review.js` wrapper 渲染，二者都不是 Editor tabs。fresh init 只显示 landing；选择工具后 header label 显示当前工具。
>
> **后端路径不随 UI 重组改变：** Download 仍使用 `/api/downloads/*`，Editor 仍使用 `/api/editor/*`，Text Review 仍使用 `/api/text-review/*`，FileTransfer 仍使用 `/api/filetransfer/*`；当前 FileTransfer 路由组在通用 `/api` 组外注册 `POST /upload` 与 `POST /path-info`，保留认证、feature gate 与 600 MiB body cap。`app.js` 在 Utility 子工具切换时调用 Download/FileTransfer 的 `suspend*`/`resume*` hooks，离开时执行各自 cleanup。
...
| 修改前端页面/资产 | PROJECT_MAP §18 | `web/static/<page>.js`、`web/static/utility/editor/*`（File Editor、Log Reader、Text Review wrapper）、`web/static/vendor/utility-editor/*`（各依赖许可证）、`web/static/index.html`、`web/static/index-nopg.html`、`web/playground/static-pg/` |
...
| 修改 Monitor Recent Requests 详情弹窗 | proxy、playground | `web/static/monitor/monitor_modal.js`（六个固定 section：`Request Info`/`Request`/`Request Headers`/`Response Headers`/`Status`/`Response Body`；默认折叠；Status 仅折叠，其余 section 级 Pretty/Raw/Copy + 字段级 Pretty/Raw/Copy；Raw 保持原始字符串；两级 sticky header）+ `web/static/info_common.js`（`renderInfoSection`/`buildInfoField` 共享兼容边界，其他调用方默认行为不变）+ `web/static/style.css`（section/field sticky 视觉承载）+ `docs/proxy-architecture.md` + `docs/playground-architecture.md` |
