...
| 文件 | 职责 |
|---|---|
| `types.go` | 配置结构体（`Config`/`Provider`/`Key`/`Combo`/`RotationConfig`/`SecurityConfig`/`AnySearchConfig` 等）+ YAML/JSON tag；`Config` 顶层新增 `QuickSlotOnly bool`（`yaml/json:"quickSlotOnly"`，控制 `/v1/models` 仅返回 QuickSlot 模型）；`AnySearchConfig` 含 `APIKey`/`MaxResults` 字段；`Provider` 新增 `AnthropicVersion`/`AnthropicBeta` 字段与 `IsAnthropic()` 方法（`APIType=="anthropic"`）；`ModelDef` 新增 `Protocols []string` 字段（yaml/json `protocols,omitempty`，记录多协议探测结果）+ `ProtocolOpenAICompat`/`ProtocolOpenAIResponses`/`ProtocolAnthropic` 常量；`Config` 顶层含 `AnySearch` 字段与 `ImageSaveDir` 字段（图片保存目录，默认 `"imgs"`） |
...
| `settings.go` | GET/PUT 设置（server/security/rotation/quickSlotOnly 等）；`getSettings` 返回 `quickSlotOnly` 字段，`updateSettings` 接收并同步到 `cfg.QuickSlotOnly` + `rt.SetQuickSlotOnly()` |
...
| 修改 QuickSlot 头部交互 / Active 联动 | PROJECT_MAP §18.2 | `web/static/quickslots.js`（`openQuickSlotModalByOrder`/`openQuickSlotModalById`/`_qsModal*` modal 系统 + capture 阶段键盘处理 + 1s 自动关闭门限 + Del 删除 + `_qsActiveId`/`qsSetActive`/`qsClearActive`/`qsGetActiveModel`/`_qsUpdateActiveClass` active 联动）、`web/static/app.js`（1-9 改调 `openQuickSlotModalByOrder(n, true)` + `isQuickSlotModalOpen()` 前置跳过）、`web/static/style.css`（`.qs-modal-overlay`/`.qs-modal`/`.qs-modal-item` 样式 + `.quickslot-btn.active` 辉光遮罩）、`web/static/i18n.js`（`qsModalHint`）、`web/playground/static-pg/pg-ui.js`（`pgApplyActiveQuickSlot` + `pgOnModelChange` 解绑 active）、`web/playground/static-pg/pg-lifecycle.js`（`renderPlayground` 套用 active 模型） |
