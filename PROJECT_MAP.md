...
| JS 模块 | `app.js`、`api.js`、`auth.js`、`i18n.js`、`info_common.js`、`providers.js`、`combos.js`、`quickslots.js`、`usage.js`、`console.js`、`terminal.js`、`monitor.js`、`download.js`、`endpoint.js`、`headerStats.js`、`shortcuts.js`（快捷键注册中心：系统预设 + 用户覆盖 + `Shortcuts.matchEvent`）、`fs-api.js`（统一文件 API：`FsApi.BlobTracker` 泄漏防护 + `pickFiles`/`pickDirectory`/`saveFile`/`collectFilesFromDataTransfer`/`walkDir`） |
| 三方 JS | `chart.umd.js` |
| 样式 | `style.css` |
