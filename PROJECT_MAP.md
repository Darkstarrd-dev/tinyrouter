...
| 文件 | 职责 |
|---|---|
| `types.go` | 配置结构体（`Config`/`Provider`/`Key`/`Combo`/`RotationConfig`/`SecurityConfig` 等）+ YAML/JSON tag；`Provider` 新增 `AnthropicVersion`/`AnthropicBeta` 字段与 `IsAnthropic()` 方法（`APIType=="anthropic"`）；`ModelDef` 新增 `Protocols []string` 字段（yaml/json `protocols,omitempty`，记录多协议探测结果）+ `ProtocolOpenAICompat`/`ProtocolOpenAIResponses`/`ProtocolAnthropic` 常量 |
| `defaults.go` | 默认配置构造 + `Finalize*` 零值回填；`finalizeConfig` 为 anthropic provider 回填 `AnthropicVersion="2023-06-01"` |
...
| 文件 | 职责 |
|---|---|
| `handler.go` | `Handler`（基于接口装配，非具体类型）：路由 `/v1/*`，构造 HTTP client（普通/流式/管理 + 代理变体）；Anthropic 入口 `Messages`（`POST /v1/messages`，`handleProxy(..., EntryFormatAnthropic)`）；OpenAI Responses 入口 `Responses`（`POST /v1/responses`，`handleProxy(..., EntryFormatOpenAIResponses)`） |
...
| `upstream.go` | `normalizeBaseURL`：剥离已知 endpoint 后缀到 API 根；`forwardUpstream` 按 `entryFormat` 三分支（OpenAI Chat / Anthropic / OpenAI Responses）；`buildAnthropicUpstreamRequest`+`setAnthropicHeaders`：`x-api-key`+`anthropic-version`(+可选 `anthropic-beta`)，URL 不注入 `/v1` 前缀，绝不设 `Authorization`；`buildResponsesUpstreamRequest`：URL 不注入 `/v1` 前缀（同 anthropic 规则），鉴权头 `Authorization: Bearer <key>`（同 OpenAI Chat） |
...
| `models.go` | 模型列表/解析辅助 |
| `recorder.go` | `recordUsage`：把完成/出错的请求写入 usage 环形缓冲；始终捕获 payload/headers（不依赖 debug mode）；大响应 `b64_json` 占位截断（见 proxy-architecture.md 2026-07-15 更新） |
...
| `probe_common.go` / `probe_model.go` / `probe_keys.go` | 单模型探测 / 全 key 探测（共享 SSE 内容提取）；`probe_common.go`/`probe_model.go` 重写为 OpenAI-compat / OpenAI-Responses / Anthropic 三协议并发复合探测，`probe_model.go` 写回 `ModelDef.Protocols` 与 `state.yaml` probes；新增 `probe_test.go` / `probe_compound_test.go` |
...
| `gallery.go` | Gallery 图片查看器后端 handlers：`galleryListZip` (POST `/api/gallery/zip` 上传 zip → 返回 `{sessionId, manifest}`)，`galleryGetZipEntry` (GET `/api/gallery/zip/{sessionId}/{entryPath:*}` 取单图字节)，`galleryConvertTiff` (POST `/api/gallery/tiff` TIFF→JPEG 转码) |
| `gallery_session.go` | Gallery zip 会话存储：包级 `gallerySessions` LRU（线程安全，上限 8 个、5 分钟空闲过期），`sync.Once` 启动后台定时清理 goroutine |
...
| 模块 JS | `pg-core`、`pg-state`、`pg-setup`、`pg-request`、`pg-stream`、`pg-render`、`pg-markdown`、`pg-modal`、`pg-ui`、`pg-lifecycle`、`pg-i18n`、`pg-director`、`pg-autochat`、`gallery.js` |
...
| 修改多协议探测/复合 Test / Responses 路由 | proxy、rotation | internal/proxy/forward.go+upstream.go+stream.go+handler.go、internal/api/probe_model.go+probe_common.go、internal/combo/resolver.go、internal/config/types.go+validate.go、internal/api/router.go、internal/state/state.go、internal/registry/models.go+state.go |
