...
server_manager.go           # HTTP 服务器优雅重启封装（端口热切换）
version.go                  # 版本号常量
...
internal/
  ...
  rotation/                 # Key 选择策略 + 冷却/退避 + NIM 支持
  combo/                    # Combo 解析 (fallback/round-robin/greedy-squirrel)
  proxy/                    # /v1/* 代理处理器 (SSE 透传)
  usage/                    # 内存环形缓冲统计 + 累计统计 + 配额追踪
  console/                  # 控制台日志捕获 + SSE 推送
  api/                      # 管理 REST API
  state/                    # state.yaml 运行时状态持久化 (去抖+定时落盘)
web/
  embed.go                  # !playground tag: 内嵌 static (不含 playground 资产)
