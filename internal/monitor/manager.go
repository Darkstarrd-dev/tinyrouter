...
func New(maxBufferLines int) *Manager {
	if maxBufferLines <= 0 {
		maxBufferLines = 500
	}
	return &Manager{
...
func (m *Manager) Start(command string, args []string, allowedCommands []string) error {
	...
	go func() {
		...
		exitMsg := fmt.Sprintf("[%s] Monitor command finished: %s", time.Now().Format("2006-01-02 15:04:05"), m.command)
		if err != nil {
