...
func (h *Handler) handleNetworkError(sel *rotation.SelectedKey, providerID, model string, err error, state *retryState, reqID string, reqHeaders http.Header, upstreamURL string) {
	...
	h.recordUsage(reqID, providerID, model, sel, "error", 0, 0, 0, 0, err.Error(), nil, nil, nil, 0, reqHeaders, upstreamURL)
...
func (h *Handler) handle429(resp *http.Response, sel *rotation.SelectedKey, providerID, model string, startTime time.Time, state *retryState, r *http.Request, reqID string, upstreamURL string) {
	...
	if sel.Provider.IsNIM() {
		...
		h.recordUsage(reqID, sel.Provider.Name, model, sel, "error", latencyMs, 0, 0, 0, bodyStr, nil, body, resp.Header, resp.StatusCode, requestHeaders(r), upstreamURL)
		return
	}
