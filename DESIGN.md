...
- `.top-header` maintains a uniform 86px height across all child components (`.top-header-nav`, `#quickslot-header`, `.top-header-brand`, `.top-header-shutdown`) and a fixed 98px header container height (`padding: 6px 16px; align-items: center; flex-wrap: nowrap !important; overflow: hidden; min-height: 98px; height: 98px`). Controls are never compressed or distorted.
- Dynamic responsive collapse (`setupHeaderResponsive()` in `auth.js`) calculates available header width at runtime instead of using hardcoded pixel breakpoints, preserving elements as long as space allows. Hiding priority order: (1) shutdown button (`.top-header-shutdown`), (2) brand logo container (`.top-header-brand`), (3) QuickSlots (`.quickslot-header` / `.top-header-stats`). The page selector (`.top-header-nav`) has guaranteed minimum visibility and maintains constant 86px height in all states.
- `.top-header-nav` is the production navigation shell: a theme-token-driven 3-column × 2-row grid containing five long rectangular `.nav-item` cells and one disabled, empty sixth button slot; outer border, frame background, box-shadow and outer padding are removed (`border: none; background: transparent; box-shadow: none; padding: 0`), preserving only the button elements. Download occupies the lower-middle slot and the lower-right slot remains empty.
- `.top-header-nav::before` and `::after` are the two small non-interactive rotated square decorations centered in the row gap between adjacent columns. In dark mode, the diamond edge facing the active cell receives the active color; in light mode, `[data-theme="light"]` hides these diamond decorations (`display: none`). `.top-header-nav-minimal` removes them and uses a single 2-cell row for the no-Playground shell.
