...
>
> **最后核对（2026-08-07 侧栏步骤 2/3 控件接线修复）：** 修复"步骤 2 单帧编辑 / 步骤 3 输出设置全部控件无效"的根因——`pageTemplate()` 已改渲染新 ID（`gif-global-crop-btn`/`gif-text-input`/`gif-text-color`/`gif-output-width`/`gif-output-height`/`gif-output-scale-slider`/`gif-output-scale-val`/`gif-sample-interval`/`gif-quality-val`/`gif-export-gif-btn`/`gif-export-frames-btn`/`gif-delete-range-btn`/`gif-keep-range-btn` 等），但入口 `cacheDom()`/`bindEvents()`/`getTargetIndices()` 与 `gif-editor-export.js` 仍按旧 ID 查找 → 所有 `if (dom.X)` 守卫静默失效、零绑定。本轮以模板为 ID 唯一事实来源对齐全部 JS；补齐模板缺失的两类控件——全局裁剪微调面板（`gif-crop-panel` + `gif-crop-slide-x/y/w/h` + `gif-crop-num-x/y/w/h` + apply/cancel，复用既有 `.gif-crop-panel.active` CSS 与 `gifEditorApplyCrop` i18n；顺带修正 `startGlobalCrop` 在 `mode='crop'` 前同步滑条的初值 bug）与精灵图导出按钮（`gif-export-sprite-btn`，接线既有 `exportSprite()`）；删除无处理死控件（`gif-export-fmt` 单选）；`syncLayerBtn`（模板已由删除图层按钮取代）及其绑定、`syncLayer()` 一并移除；新接线：描边色（`gif-text-stroke-color`，drawLayers strokeText + addText + 选中同步）、图层缩放滑条（`gif-layer-scale`，基于 `layerBase` 基线：文字 size / 图片 w·h，选中/命中时重置为 1）、删除图层（`gif-delete-layer-btn`）、百分比应用（`gif-scale-percent` + `gif-apply-scale-btn`，输出尺寸按百分比计算，修正输出缩放滑条原 100 视为 100× 的 bug）；范围作用域改写为 `gif-crop-start`/`gif-crop-end` 双输入（校验同 `getBatchRange`），`gif-crop-range-inputs` 随单选切换显示；质量滑条联动只读 `gif-quality-val` span。i18n 补 `gifEditorCancelCrop`/`gifEditorCropX/Y/W/H`/`gifEditorStroke`/`gifEditorDeleteLayer`/`gifEditorApplyAll`/`gifEditorExportSprite`/`gifEditorColor`（en/cn）。浏览器冒烟（playground 构建 localhost:20128，headless Chrome）全过：导入→2×2 切片→范围加文字/描边/缩放/删除→全局裁剪（面板/滑条/画布拖动 +20px 精确）→输出缩放 2.0x/1.5x/0.9x（100%=1.0x）→质量滑条→GIF / ZIP(Archive pack，PK 头校验) / 精灵图三导出→批量删除/保留范围，0 console error。
>
...
- [x] 原单页功能 1:1 还原（**2026-08-06 下午修复后成立**；步骤 2/3 控件接线见 2026-08-07 顶部最后核对）：图片/GIF/视频三类输入（Import Modal）、网格切片（`gif-rows`/`gif-cols`/`gif-slice-btn`）、时间轴（选中/复制/删除/延迟/拖拽排序/键盘导航）、全局裁剪（`gif-global-crop-btn` + crop-panel 微调滑条 + gizmo）、图层（文字/贴图/作用范围/描边/缩放/删除）、色键透明（`gif-enable-trans` + 取色/容差）、批量延迟/删帧/范围保留、导出 GIF / 序列帧 ZIP / PNG 精灵图（含输出宽高/缩放/质量参数、拼图导出按钮）。
...
| 输出 | 方案（当前工作树实现） | 细节 |
|---|---|---|
| GIF | gif.js 编码（`gif-editor-export.js::exportGif`） | `ensureGifJs` 确保 vendor 就绪（60s 兜底超时）；`new GIF({workers: 4, quality: clamp(1..10), width, height, workerScript: GIF_WORKER_URL})`；quality 读 `#gif-quality-val`（模板滑块联动，默认 10）；色键透明开启且 `transparencyReady` 时设 `transparent: MATTE_NUM`（`#FF00FF` matte）；逐帧 `renderCompositedFrame(…, {applyTransparency, matte})` + `delay = max(1, round(slice.delay))`；finished → `showResultModal(blob, 'Slice_<ts>.gif', 'image/gif', 'gif')`（结果弹窗 = 预览 `<img>` + Download anchor + 「Open in Gallery」） |
...
- **1:1 编辑功能（还原）**：色键透明（取色/容差/取消）、边缘裁剪 + 网格切片（`gif-slice-btn`→`runSlice`）、全局裁剪（`gif-global-crop-btn` + crop-panel 微调滑条 + gizmo）、图层（文字/贴图/作用范围/描边/缩放/删除）、批量延迟、删帧/仅保留范围、输出宽高/缩放（`gif-output-scale-slider`，100%=1.0x，可经 `gif-scale-percent` + `gif-apply-scale-btn` 百分比应用）/质量（1-10 滑块联动）、拼图 Sheet 行/列参数 + `gif-export-sprite-btn` 导出按钮；导出按钮初始 disabled、导入后启用。
