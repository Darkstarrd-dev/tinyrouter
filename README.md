...
- **添加**：Settings 页面 → Quick Slots → 新建。填写名称、`order` (1~4)、并从 Provider 导入模型。
- **绑定数字键**：`order` 决定对应哪个数字键（1→order 1，2→order 2，依此类推）。
- **循环切换**：按下对应数字键，在当前槽「已启用」的模型之间循环切换；被停用的模型会自动跳过。
- **当前选中**：顶部 header 会显示该槽的名称与当前模型的 Provider 前缀，点击可下拉直接选择具体模型。
- **编辑**：在槽的编辑弹窗中可增删模型、拖拽排序；切换模型时右上角只会保留最后一次提示，不会堆叠。
...
|---|---|
| `F1` | 切换到 Usage 统计页面 |
| `F2` | 切换到 Settings (Endpoint) 页面 |
| `F3` | 切换到 Console 日志页面 |
| `F4` | 切换到 Playground 页面 |
| `1` ~ `4` | 循环切换对应 `order` 的 Quick Slot 模型 |
| `Esc` | 关闭弹窗；若没有弹窗则关闭服务器 |

> 数字键 `1`~`4` 仅在未聚焦输入框 / 无弹窗时生效，避免与正文输入冲突。
